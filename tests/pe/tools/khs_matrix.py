...
def main():
    ...
    with pathlib.Path(a.csv).open(encoding="utf-8") as fh:
    ...
    if not rows:
        print(
            "  no rows at target %s%s" % (a.target, " for %s" % a.rtl if a.rtl else "")
        )
        return 1
