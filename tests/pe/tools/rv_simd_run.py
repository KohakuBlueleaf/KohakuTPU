...
ROW = re.compile(
    r"@@@ DSP (\d+) cycles (\d+) kinstr (\d+) retired (\d+) " r"total (\d+) (\S+)"
)
...
def main():
    ...
    if (
        not a.no_gen
        and subprocess.run(
            [sys.executable, str(TOOLS / "rv_simd_gen.py"), "--simd", str(a.simd)],
            cwd=str(ROOT),
            check=False,
        ).returncode
    ):
                ...
                w.writerow(
                    {
                        ...
                        "simd": a.simd,
                        "name": x["name"],
                        "net_cycles": x["net_cycles"],
                        "kinstr": x["kinstr"],
                        "cpi": x["cpi"],
                        "raw_cycles": x["cycles"],
                        "retired": x["retired"],
                        "total_cycles": x["total"],
                        "verdict": x["verdict"],
                        "note": x["note"],
                    }
                )
