...
// Two-clock copy of tests/matmul/mx_cluster_node_tb.v.

// -d MX_BASE swaps in the shipping single-clock mx_cluster_node at the SAME 1x
// period, so the pump's speed-up is timed against what ships, not against a PoC.
module mx_cluster_node_pump_tb;
    ...
    // GATED, as ktpu_div2 models BUFGCE_DIV: a `<=` toggle divider puts clk in
    ...
    // pumped pair's out-of-range second fetch reads past the fill, so it is zero.
`ifdef MX_L1WRAP
    // OFFSET 255: the pair's +1 must wrap inside the bank as `a_off8` does, not
    // carry into the bank bit and read the other buffer.
    localparam integer L1GA = 512, L1GB = 512;
    localparam [7:0]   AOFF = 8'd255, BOFF = 8'd255;
    localparam         ABANK = 1'b0, BBANK = 1'b0;
`elsif MX_L1OFF
    localparam integer L1GA = 512, L1GB = 512;
    localparam [7:0]   AOFF = 8'd16, BOFF = 8'd16;
    localparam         ABANK = 1'b1, BBANK = 1'b1;
`else
    localparam integer L1GA = 32, L1GB = 32;
    localparam [7:0]   AOFF = 8'd0, BOFF = 8'd0;
    localparam         ABANK = 1'b0, BBANK = 1'b0;
`endif

    reg          l1_we = 0, l1_sel = 0;
    reg [15:0]   l1_addr = 0;
    // SIZED: an unsized sum in a concatenation contributes 32 bits, not 8.
    reg [7:0]    loff = 0;
    reg [927:0]  l1_data = 0;
    reg          gemm_start = 0;
...
`ifdef MX_BASE
    mx_cluster_node #(.TILES(256), .GA(L1GA), .GB(L1GB),
                      .ACC_MW(MW), .MODEL(MODEL)) dut (
        .clk(clk), .rst(rst),
`else
    mx_cluster_node_pump #(.TILES(256), .GA(L1GA), .GB(L1GB),
                      .ACC_MW(MW), .MODEL(MODEL)) dut (
        .clk1x(clk), .clk2x(clk2x), .rst(rst),
`endif
        ...
        .gemm_aoff(AOFF), .gemm_boff(BOFF), .gemm_emit(1'b0),
        .gemm_abank(ABANK), .gemm_bbank(BBANK),
    ...
    // Wall-clock, not cycles: once the domains differ, cycle counts are not
    // comparable between builds. `gemm_ns` accumulates only the sweep itself.
    real gemm_ns = 0.0;
    real t_sweep;
    integer n_sweep = 0;
    ...
    task run_case(input integer gm, input integer gn, input integer nk,
        ...
        begin
            ...
            for (ch = 0; ch < chunks; ch = ch + 1) begin
                ...
                gemm_ns = gemm_ns + ($realtime - t_sweep);
                n_sweep = n_sweep + 1;
            end
...
`ifdef MX_SPEED
        // Large NK so the K loop dominates the ~19-deep cascade. Bounded by
        // L1: gm*nk <= GA and gn*nk <= GB, both 32 in this bench.
        run_case(4, 4, 8, 1);
...
`elsif MX_FAST
  `ifdef MX_FAST4
        run_case(2, 2, 3, 1);       // four sub-tiles, odd NK > 1
  `elsif MX_FAST3
        run_case(2, 1, 2, 1);       // two sub-tiles, even NK
  `elsif MX_FAST2
        run_case(2, 1, 1, 1);       // two sub-tiles, odd NK
  `else
        // One even-NK sweep: the smallest case that exercises a full pump pair.
        run_case(1, 1, 2, 1);
  `endif
        $display("  SPEED -- %0d sweeps, %0.1f ns total, %0.2f ns/sweep",
                 n_sweep, gemm_ns, gemm_ns / n_sweep);
...
`endif
        ...
        run_case(1, 2, 1, 1);
        run_case(3, 1, 1, 1);
        // 4 sub-tiles at nk=1: Gm*Gn below REUSE_MIN while `pace_n` is 0
