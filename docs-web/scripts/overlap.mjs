...
const audit = () => {
  ...
  for (const [si, svg] of [...document.querySelectorAll("svg.dgm")].entries()) {
    ...
    for (const t of texts) {
      ...
      if (host && over > 1)
        out.push({ svg: si, kind: "overflow", a: t.s, by: over });
    }
  }
  ...
  // A sheet (Fig `sheet`) is tall by design and is read in the full-screen
  // viewer, so its aspect is not a finding.
  for (const f of document.querySelectorAll(
    "figure.doc-break:not([data-sheet])",
  )) {
    ...
    if (r.height > r.width * 0.5)
      out.push({
        kind: "tall-wide",
        a: (f.querySelector("figcaption")?.textContent || "")
          .trim()
          .slice(0, 52),
...
for (const r of routes) {
  ...
  const body = await page.evaluate(() => {
    ...
    return {
      ok: !!m?.querySelector("article, .container-page"),
      h: m?.scrollHeight ?? 0,
    };
  });
  if (!body.ok) {
