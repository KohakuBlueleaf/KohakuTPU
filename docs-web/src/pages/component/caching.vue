...
<template>
  ...
  >
    <p class="doc-p">
      This machine has <b>no cache in the datapath</b>. It has on-chip memory
      you address, a conversion stage the mover drives, and a compiler that
      knows which units want the same bytes. That is a deliberate position, and
      the honest version of it is that
      <b
        >any caching proposal here has to say what it adds beyond shared
        fetch</b
      >.
    </p>
    ...
    >
      <p>
        ...
        serve a 1,024-bit line in one port-A read with no burst, and why a
        staged fetch never transforms —
    ...
    >
      <p>
        ...
        sysnode's single-master concentrator converges
      ...
      <p>
        Each instance measured <b>4,490 LUT and 32 DSP</b>. The reason no bench
        caught it is worth keeping too: the mesh bench set up a transformed
        fetch and then only checked <i>“FILL completed”</i>, so it passed
        unchanged either side of the removal.
      </p>
    </Callout>
    ...
    <Callout
      kind="measured"
      title="The transform was also the path setting WNS"
    >
      <p>
    ...
    <Callout kind="note" title="Why that matters more than hit rate">
      <p>
        ...
        <code>MEM_RD_REQ</code>; it emits loads and stores. A tier that turns an
        address in one class into a flit gives it the protocol without the core
      ...
      <p>
        ...
        design question this machine has not had to answer yet, because its own
