...
const stream = {
  nodes: [
    ...
    { id: "fetch", x: 15, y: 4.6, w: 11, label: "fetch", sub: "f1 → f2, 3 deep" },
    {
      id: "dec",
      x: 15,
      y: 9.2,
    ...
    { id: "l0", x: 0, y: 18.5, w: 8, label: "lane 0", sub: "x0..x31 · ALU · FMA" },
    { id: "l1", x: 9.2, y: 18.5, w: 8, label: "lane 1", sub: "x0..x31 · ALU · FMA" },
    { id: "l2", x: 18.4, y: 18.5, w: 8, label: "lane 2", sub: "x0..x31 · ALU · FMA" },
    { id: "ld", x: 27.6, y: 18.5, w: 4.5, label: "…" },
    { id: "l7", x: 33.6, y: 18.5, w: 8, label: "lane 7", sub: "x0..x31 · ALU · FMA" },
  ],
  edges: [
...
const floatPath = {
  nodes: [
    ...
    { id: "half", x: 13, y: 9.8, w: 11, label: "half = f7[2]", sub: "per instruction", accent: true },
    { id: "hp", x: 26, y: 9.8, w: 13, label: "hpipe, 15 deep", sub: "the width follows the result" },
  ],
  edges: [
...
const conversions = {
  ...
  rows: [
    { c: "FP16 → E8M15", p: "<b>exact</b> — nothing is lost, and a subnormal normalises into an ordinary E8M15 value", _tone: "good" },
    {
      c: "FP32 → E8M15",
...
const renderMix = {
  ...
  rows: [
    ...
    },
    {
      ...
      needs: "wrap, clamp, mip select, Morton swizzle — <code>y*width+x</code> is why RV32M is built",
      kind: "<b>integer / bitwise</b>",
    },
    { stage: "texture filtering", needs: "fixed-point or E8M15 weights", kind: "float-ish" },
    {
      stage: "fragment / colour shading",
      needs: "mediump — E8M15 exceeds fp16 in range and mantissa",
      kind: "<b>float</b>",
    },
    { stage: "vertex transform", needs: "E8M15 products into an FP32 accumulator", kind: "<b>float</b>" },
  ],
}
...
const groups = {
  ...
  rows: [
    { g: "SALU", enc: "custom-2, funct3 0", n: "10", what: "scalar ALU, register-register" },
    { g: "SMOV", enc: "custom-2, funct3 1", n: "2", what: "<code>s2v</code>, <code>rdctl</code>" },
    {
      g: "DIV",
...
const fltOps = {
  ...
  rows: [
    ...
    { i: "vfmul vd, vs1, vs2", d: "vd = vs1 * vs2", how: "the lane with its addend forced to 0.0" },
    { i: "vfadd vd, vs1, vs2", d: "vd = vs1 + vs2", how: "the lane with its multiplier forced to 1.0" },
    {
      i: "vfsub vd, vs1, vs2",
      d: "vd = vs1 - vs2",
...
const vmemOps = {
  ...
  rows: [
    ...
    { op: "4", stem: "vlinu", what: "lane-linear load, unsigned", _tone: "good" },
...
const tiers = {
  ...
  rows: [
    ...
    },
    {
      ...
      knows: "the high bits are equal, so the compare is over <b>offset fields</b> rather than full computed addresses",
      req: "1..LANES",
    },
    {
...
const regions = {
  nodes: [
    { id: "ea", x: 15.5, y: 0, w: 12, label: "ea", sub: "one per LANE", accent: true },
    { id: "dram", x: 0, y: 5.5, w: 12, label: "R_DRAM", sub: "ea[31] = 1 · via rv_l1" },
    { id: "on", x: 14.5, y: 5.5, w: 14, label: "ea[31] = 0", sub: "ea[30:28] selects" },
...
const gates = {
  ...
  rows: [
    ...
    { g: "G1", gen: "WAVES 16", adds: "wave-indexed storage — many wave contexts", built: "yes", _tone: "good" },
    { g: "G2", gen: "HAS_MASK 1", adds: "the active mask, <code>tmc</code>, predication", built: "yes", _tone: "good" },
    {
      g: "G3",
      gen: "HAS_IPDOM 1",
    ...
    { g: "G5", gen: "—", adds: "the coalescer: one gather becomes one request when lanes agree", built: "no", _tone: "warn" },
    { g: "G6", gen: "—", adds: "MSHRs: more than one miss in flight", built: "no", _tone: "warn" },
    {
      g: "G7",
...
const ladder = {
  ...
  rows: [
    { g: "G0 the arithmetic substrate", w: "1", m: "0", i: "0", lut: "2,952", d: "—", ff: "307", bram: "8", cs: "2", f: "324.1 MHz" },
    { g: "G1 wave-indexed storage", w: "16", m: "0", i: "0", lut: "2,952", d: "<b>+0</b>", ff: "311", bram: "8", cs: "2", f: "324.1 MHz" },
    { g: "G2 active masks, <code>tmc</code>", w: "16", m: "1", i: "0", lut: "3,016", d: "+64", ff: "447", bram: "8", cs: "18", f: "324.1 MHz" },
    { g: "G3 <code>split</code>/<code>join</code>, bounded stack", w: "16", m: "1", i: "1", lut: "3,204", d: "+188", ff: "516", bram: "8", cs: "36", f: "324.1 MHz" },
  ],
}

const deltas = [
  { label: "G1 — WAVES 1 → 16, sixteen wave contexts", value: 0, note: "+0 LUT, +4 FF", tone: "good" },
  { label: "G2 — HAS_MASK, the active mask and tmc", value: 64, note: "+64 LUT", tone: "good" },
  { label: "G3 — HAS_IPDOM, split/join and the bounded stack", value: 188, note: "+188 LUT", tone: "good" },
  { label: "every SIMT gate on this sweep, G3 − G0", value: 252, note: "on a substrate of 2,952", tone: "accent" },
]
...
const laneScale = [
  { label: "LANES = 4", value: 1659, note: "415 LUT/lane · 36 ctrl sets · 324.1 MHz" },
  { label: "LANES = 8", value: 3204, note: "401 LUT/lane · 36 ctrl sets · 324.1 MHz", tone: "accent" },
  { label: "LANES = 16", value: 6355, note: "397 LUT/lane · 36 ctrl sets · 324.0 MHz" },
  { label: "LANES = 32", value: 12478, note: "390 LUT/lane · 36 ctrl sets · 324.1 MHz" },
]
...
const vregPrim = {
  ...
  rows: [
    { p: "block <span class='opacity-60'>(default)</span>", lut: "3,204", lram: "20", ff: "516", bram: "8", cs: "36", f: "324.1 MHz" },
    { p: "distributed", lut: "9,430", lram: "5,140", ff: "1,028", bram: "<b>0</b>", cs: "165", f: "475.3 MHz" },
    { p: "<b>Δ</b>", lut: "<b>+6,226</b>", lram: "+5,120", ff: "+512", bram: "−8", cs: "+129", f: "<b>+151.2 MHz</b>", _tone: "bad" },
  ],
}
...
const crossLane = {
  cols: [
    ...
    { key: "bfly", label: "G8 butterfly · N log N", align: "right", mono: true },
  ...
  rows: [
    ...
    { l: "16", bfly: "+3,019", res: "6,194", r: "2.1×", f4: "<b>339.2 MHz</b>", _tone: "warn" },
    { l: "32", bfly: "+6,000", res: "25,961", r: "<b>4.3×</b>", f4: "<b>317.7 MHz</b>", _tone: "bad" },
  ],
}
...
// ---------------------------------------------------------------------------
const budget = [
  { label: "controller PE — rv_pe, SIMD_EN = 0", value: 2477, note: "measured · 2.857 ns" },
  { label: "kht_unit at G3 — the SIMT unit alone, integer lanes", value: 3204, note: "measured · 3.333 ns" },
  { label: "kht_core — the pipeline, kht_unit inside it", value: 9653, note: "measured · 3.333 ns" },
  { label: "SIMD PE — SIMD 8 + 4 float lanes", value: 13772, note: "measured · 2.857 ns" },
  { label: "SIMT PE, integer only — the campaign's baseline", value: 15794, note: "measured · 2.500 ns", tone: "warn" },
  { label: "SIMT PE, 8 int + 8 float lanes, RV32M — OF RECORD", value: 21586, note: "measured · 2.857 ns", tone: "accent" },
  { label: "target band, upper edge", value: 25000, note: "TARGET · the band is 20–25k", tone: "good" },
...
const record = {
  cols: [
    { key: "k", label: "kht_pe · 8 lanes / 16 waves · xcvu13p-fhgb2104-2L-e · OOC synth at 2.857 ns" },
  ...
  rows: [
    ...
    { k: "FF", v: "17,268" },
    { k: "BRAM", v: "30.5" },
    { k: "DSP48", v: "<b>48</b> <span class='opacity-60'>— 2/lane float, 4/lane multiply</span>" },
...
const runOut = {
  cols: [
    { key: "k", label: "One shader through SIMT PE + router + MAG + RAM, 8 lanes × 16 waves" },
    { key: "v", label: "", mono: true, align: "right" },
  ],
  rows: [
    { k: "halt word", v: "00000055 <span class='opacity-60'>(model 00000055)</span>", _tone: "good" },
    { k: "halt cause", v: "1 <span class='opacity-60'>(model 1)</span>", _tone: "good" },
...
const ldsWitness = {
  ...
  rows: [
    { g: "0 <span class='opacity-60'>serial walk</span>", r: "<b>48</b>", ga: "6", res: "PASS" },
    { g: "1 <span class='opacity-60'>banked</span>", r: "<b>34</b>", ga: "6", res: "PASS", _tone: "good" },
  ],
}
</script>
...
<template>
  ...
  >
    ...
    <p class="doc-p">
      A wave is <b>one instruction stream driving N lanes</b>. There is one program
      counter per wave, one fetch, one control word — and N copies of the register
      file, N ALUs, N addresses. Everything expensive about instruction supply is
      paid once; the thing that is replicated is the arithmetic and the state it
      works on.
    </p>
    ...
    <Fig
      ...
      zoom
    >
      <BlockDiagram :nodes="stream.nodes" :edges="stream.edges" :groups="stream.groups" />
    </Fig>
    ...
    <Fig caption="add x5, x3, x4 — for every active lane, x5[lane] = x3[lane] + x4[lane].">
    ...
    <p class="doc-p">
      ...
      property: <code>x1</code>–<code>x31</code> <i>are</i> the per-lane registers.
      No new register class exists in the base ISA, so no compiler fork is needed to
      allocate one.
    </p>
    ...
    <Fig
      ...
      zoom
    >
      <BlockDiagram :nodes="halves.nodes" :edges="halves.edges" :groups="halves.groups" />
    </Fig>

    <h2 class="doc-h2">What SIMT costs, and what it bought</h2>
    <p class="doc-p">
      These are the things this PE has that the
      <RouterLink to="/component/simd" class="doc-link">SIMD tier</RouterLink> does not. The
      SIMD PE goes wide on work that is <i>uniform</i>; this one exists for the case
      where lanes need to <b>disagree</b>.
    </p>
    ...
    <Callout kind="measured" title="The configuration of record, and all of it is built">
      <p>
        <b>8 integer lanes and 8 float lanes, with RV32M integer multiply.</b> It is
        stated here because a baseline nobody names gets silently assumed to be
        something else — and an earlier revision of this page said the reference was
        8 int + 4 float and that neither number was built. Both halves of that are
        false now.
      </p>
    </Callout>
    ...
    <h3 class="doc-h3">Why the two numbers are equal now, and what still pins the integer one</h3>
    ...
    <p class="doc-p">
      ...
      eight threads is exactly one <code>MEM_RD_REQ</code>, and that is the strongest
      machine-level alignment in the design — narrow the integer side and every
      coalesced load becomes two or more requests, for every kernel, permanently.
      Float has no such constraint: it is pure arithmetic, deeply pipelined at II = 1,
      and sixteen resident wave contexts hide its 15-cycle latency rather than
      stalling on it. <b>Eight integer lanes is a constraint; eight float lanes is a
      choice</b>, and what the knob was turned <i>to</i> is set by the mesh:
      <code>8×4 + 4×8 = 64</code> FP FMA per clock is one Mali-G610 shader core, and
      at four float lanes the same mesh is 48 and short.
    </p>

    <Callout kind="trap" title="FLANES < LANES returns ZERO in the upper lanes, and zero is a plausible float answer">
      <p>
        <code>kht_fpu</code>'s <code>g_nolane</code> assigns <code>32'd0</code> to
        every lane above <code>FLANES</code>, because there is no walk sequencer to
        feed them. A shader run on a reduced build gets a <b>silently wrong result
        rather than a fault</b> — and zero is a number a float kernel meets
        constantly, so nothing downstream trips on it either.
      </p>
      <p>
        This is the one place in this PE that breaks its own rule that a build which
        cannot do something faults instead of answering plausibly, and it is guarded
        by <b>convention only</b>: <code>FLANES</code> must equal <code>LANES</code>
        in any build that runs a shader, the configuration of record is
        <code>FLANES = 8</code>, and every shader in the suite runs against that.
        Treat a reduced build as an area measurement and never dispatch to one.
      </p>
    </Callout>

    <h3 class="doc-h3">The float tier takes both operand widths, and that is not a knob</h3>

    <Callout kind="rule" title="Operand width is a property of the INSTRUCTION, not of the build">
      <p>
        The funct7 bit that distinguishes <code>vfma</code> from <code>vfma_h</code>
        drives <code>half</code> in <code>kht_fpu</code>, which drives
        <code>wide(!half)</code> into the lane. <code>wide</code> is a <b>port</b> on
        <code>khs_float_lane</code>, not a parameter, and that lane's own header states
        the contract:
      </p>
      ...
      <p>
        ...
        two widths. What <i>is</i> a parameter is whether a float tier exists at all
        (<code>HAS_FLT</code>) and how many lanes it has (<code>FLANES</code>).
        Neither selects a format.
      </p>
    </Callout>
    ...
    <Callout kind="note" title="Where the arithmetic comes from, and why it is not forked">
      <p>
        Every float lane is one <code>khs_float_lane</code> — the
        <RouterLink to="/component/simd" class="doc-link">SIMD tier</RouterLink>'s, verbatim.
        <code>kht_fpu</code> selects operands, drives the width bit and converts the
        result back; <b>it does not compute</b>. That is what makes a GPU float number
        comparable to a DSP float number, and what keeps
        <code>cost(SIMT) = G8 − G0</code> meaning anything at all. The SIMD tier
        instantiates the same lane with <code>.wide(1'b0)</code> and never raises it,
        because its float unit is an <i>accumulator</i> and the operand width changes
        FSLOTS, the partial count and the fold order — architecture, not a port
        widening.
      </p>
    </Callout>
    ...
    <p class="doc-p">
      Float is <b>not optional</b> for this PE — rendering needs it. But the integer
      half is not a leftover from a compute machine either: rasterisation and depth
      are integer <b>because float gets them wrong</b>. Those are exactness
      requirements, not performance ones.
    </p>
    ...
    <p class="doc-p">
      Two custom majors, six groups. <code>custom-2</code> (0x5B) carries the R-type
      groups, where <code>funct3</code> names the group and <code>funct7</code> the
      operation; <code>custom-3</code> (0x7B) carries the I-type ones, where
      <code>funct3</code> names one instruction and the immediate is 12 bits.
    </p>

    <Fig caption="custom-2, R-type. funct3 names the group; funct7 names the operation inside it — eight groups of up to 128.">
      <BitField :fields="rtype" />
    </Fig>

    <Fig caption="custom-3, I-type. There is no funct7, so an I-type group holds exactly ONE instruction — the SIMD tier hit this wall and spent a whole funct3 on vld and another on vst. Splitting R from I across two majors buys eight of each instead of eight in total.">
    ...
    <Fig caption="The FLT funct7. One bit picks the operand width and two pick the operation, so the datapath slices the field instead of comparing against one constant per encoding — the same trick the vmem group uses.">
    ...
    <Callout kind="rule" title="A depth of D permits D/2 nested levels, not D−1">
      <p>
        A <code>split</code> pushes <b>two</b> entries and a <code>join</code> pops
        one. The resume PC is always the popping join's own <code>pc+4</code>, so a
        stack entry is <code>LANES</code> bits and carries no PC — exact for
        structured control flow, which SPIR-V guarantees by naming a merge block for
        every selection and loop. An earlier plan said D−1 and was wrong; the
        statement now appears in the table, in the generated header, in the model and
        in the RTL.
      </p>
      <p>
        <b>Overflow is a fault</b> — not a wrap, not a mask merge, not a truncation.
        A masked-off lane that silently reactivates is a wrong answer with no witness.
        Underflow, a <code>join</code> on an empty stack, is the same fault.
      </p>
    </Callout>

    <Callout kind="trap" title="bar encodes, does not execute, and does not fault either">
      <p>
        <code>kht_predec</code> sets <code>C_BAR</code>, and nothing in
        <code>kht_core</code> or in the golden model reads it — so a workgroup barrier
        <b>retires as a no-op</b>. Every other unbuilt thing in this ISA raises a
        fault, which makes this the single exception to the rule and the one that can
        produce a wrong answer with no witness. With one wave per workgroup the no-op
        happens to be correct; with more than one it is a race.
        <b>Do not write a shader that relies on it.</b>
      </p>
    </Callout>

    <h3 class="doc-h3">What has no encoding, deliberately</h3>
    <p class="doc-p">Each of these is an absence with a reason, not an omission.</p>

    <Callout kind="rule" title="RV32I conditional branches are reserved and illegal in shader code">
      <p>
        A branch reading a masked per-thread condition into a single PC is undefined.
        <i>“Legal only when the compiler proved it uniform”</i> is an unfalsifiable
        contract across four consumers, so <b>the encoding refuses instead</b> and the
        hardware raises an illegal-instruction fault. Uniform control uses
        <code>sbeqz</code>/<code>sbnez</code>; divergent control uses
        <code>split</code>/<code>join</code>. <code>jal</code> and <code>jalr</code>
        stay ordinary RV32I and are wave-wide: a call is uniform by construction.
      </p>
    </Callout>

    <Callout kind="note" title="No divide, no atomics, no gather opcode, no tex">
      <p>
        <b>Divide and remainder</b> stay illegal — <code>funct3</code> 100–111 in the
        RV32M group fault. A long sequential unit does not suit a barrel-scheduled
        pipeline whose whole invariant is a fixed latency, and divide-by-a-constant
        strength-reduces to <code>mulhu</code>, which now exists.
      </p>
      <p>
        <b>Atomics</b> are not in the legal opcode set at all, so an
        <code>amo*</code> word raises an illegal-instruction fault rather than being
        decoded into something adjacent.
      </p>
      <p>
        ...
        <code>vmem</code> group is not a gather — it is the <i>uniform-base</i> case,
        handed to the coalescer instead of rediscovered by it. And there is no
        <code>tex</code> because address math is integer, the fetch is an ordinary
        load and filtering is FMAs.
      </p>
    </Callout>
    ...
    <Callout kind="rule" title="An all-zero active mask is not a defined case">
      <p>
        ...
        cannot happen — and that assertion is what named a real bug in one line. See
        <RouterLink to="/mpe/simt/microarchitecture" class="doc-link">Trap 6</RouterLink>.
      </p>
    </Callout>
    ...
    <Fig caption="The vmem funct7, packed rather than enumerated: funct7 = op&lt;&lt;4 | scale&lt;&lt;2 | width. The datapath SLICES it instead of comparing against one constant per encoding, and op &gt;= 3 is exactly the lane-linear predicate — one comparator.">
    ...
    <Callout kind="note" title="A uniform base does not imply contiguous offsets">
      <p>
        <code>s[ss1] + (v[vs2] &lt;&lt; scale)</code> with arbitrary per-lane offsets
        is still a scatter and still needs the full leader/follower pass. What the
        uniform-base form buys is narrower and real: the coalescer compares
        <b>offset fields</b> rather than full computed addresses, and it knows the
        high bits cannot differ. The compare is narrower, <b>never skipped</b> —
        including when <code>ss1</code> is <code>s0</code>, which is legal and
        degenerates to a pure vector address.
      </p>
      <p>
        Two semantics pinned so the RTL and the model cannot drift: offsets are
        <b>signed</b>, and <code>s + (v &lt;&lt; scale)</code> <b>wraps at 32 bits</b>,
    ...
    <p class="doc-p">
      Every lane produces its own effective address, and the region it lands in is
      decided by a decode on the address itself — there is no tag lookup on the way
      to a window, because the window <i>is</i> the home of those addresses.
    </p>
    ...
    <p class="doc-p">
      The SIMT PE exists to answer one question: <b>what does SIMT cost on this
      fabric?</b> A number produced by building a SIMT core and comparing it to
      ...
      time. So the design is arranged so the answer is an identity.
    </p>

    <Callout kind="rule" title="cost(SIMT) = G8 − G0">
      <p>
        On one fabric, one lane lineage, one memory system. G0 and G8 are the same
        module with different generics; everything between them is a <b>controlled
        difference</b> — one gate turns one parameter, and its delta is attributable
        to that parameter and nothing else.
      </p>
      <p>
        <b>The ladder is parameters, not branches.</b> A gate that is not built must
        elaborate <i>none</i> of its logic. If <code>HAS_IPDOM = 0</code> left a stack
        in the netlist with its enables tied low, the tool would trim some of it, keep
        some of it, and the G3 delta would be the cost of the parts it happened to
        keep. That is what makes <i>“the total is the sum of the measured deltas”</i>
        true rather than intended — and it is why there is one synthesis script for
        the whole ladder.
      </p>
      <p>
        ...
        correctly inside a generate, but its <i>writeback mux input</i> was not, so
        28 LUT of a switched-off gate survived. The fix is to make the select
        constant-false at elaboration. Worth 28 LUT? No. Worth the property? Yes — a
        rule that holds “mostly” is not one you can add up.
      </p>
    </Callout>
    ...
    <Callout kind="measured" title="G1 is free, and it is the most load-bearing measurement here">
      <p>
        <b>Sixteen wave contexts cost +0 LUT, +0 BRAM, +0 control sets and +4 FF.</b>
        Not “almost nothing” — nothing, to within four flops of the wave-id register.
        A RAMB18E2 in simple-dual-port is 512 × 36, so 32 registers × 16 waves fills
        one exactly at 32 of 36 useful bits: up to sixteen waves the marginal BRAM
        cost of a wave is <b>zero</b>, and the count buys latency tolerance rather
        than storage.
      </p>
      <p>
        That matters because “many resident contexts to hide memory latency” is the
        part of SIMT that sounds expensive and is the whole reason the model tolerates
        a cache miss.
      </p>
    </Callout>
    ...
    <Callout kind="measured" title="Fmax never moves across G0–G3">
      <p>
        Every gate lands at <b>324.1 MHz</b>, identically. The SIMT machinery — the
        mask, the stack, the wave indexing — is not on the binding path at eight
        lanes; the binding path is in the lane array, where it was at G0. The usual
        objection to SIMT is that divergence tracking is on the critical path. At this
        width, on this fabric, it measurably is not.
      </p>
    </Callout>
    ...
    <Callout kind="trap" title="G3 was over its bracket, and why it is not now">
      <p>
        ...
        <code>WAVES × IPDOM_D</code> entries behind a wide read mux and an equally wide
        write decoder. It came in far over its <code>&lt;+1k</code> bracket — a shape
        this project has been billed for twice before, at 29,409 LUT and at 701, both
        fixed the same way. Rebuilt as a <code>kohaku_sdpram</code> in
        <code>distributed</code> mode with <code>READ_LAT 0</code>, the whole gate is
        <b>+188 LUT</b>, and the sweep shows where it went: G3 is the only row with
        <code>lut_mem</code> non-zero, at <b>20 LUT</b> of distributed RAM — the stack
        itself. The rest is pointer, phase and fault logic, visible as control sets
        going 18 → 36.
      </p>
    </Callout>
    ...
    <p class="doc-p">
      Not a ladder gate — the lane count is the design's biggest free variable, so it
      is measured rather than extrapolated from the 8-lane row.
    </p>
    ...
    <p class="doc-p">
      ...
      thirty-two: the active mask is a write enable per bank and the divergence state
      is per wave, not per lane, so widening the array adds datapath and adds no
      control. <b>The intercept is ~112 LUT</b>: everything that is not a lane is
      about a hundred LUT, so at any useful width this unit <i>is</i> its lane array.
      And <b>Fmax does not move either</b> — but that one came with an expiry date,
      and it has been collected on.
    </p>

    <h3 class="doc-h3">The two cross-lane networks, and why complexity class is the argument</h3>
    ...
    <Callout kind="measured" title="The flat 324 MHz did not survive a cross-lane network, exactly as the caveat said">
      <p>
        Lane scaling is flat at 324 MHz from 4 lanes to 32 <b>because there is no
        cross-lane network in <code>kht_unit</code></b> — lanes are independent, so a
        wider array is wider and not deeper. G4 is the first cross-lane network, and
        at 32 lanes it runs at <b>317.7 MHz, below the rest of the unit</b>: the
        resolver becomes the binding path. G8 is the second, costing 20–39 MHz at
        every width but <i>not</i> getting worse with width, because its depth grows
        as log while everything it competes with grows faster.
      </p>
      <p>
        <b>32 lanes is out.</b> <code>kht_unit</code> and <code>kht_lds</code> alone
        are 38,439 LUT there, past the 35k ceiling before the core, the L1, the
        requestor or the port is counted — and they do it while missing the clock. If
        16 lanes is ever wanted the answer is a <b>cheaper resolver</b>, not a bigger
        budget.
      </p>
    </Callout>
    ...
    <p class="doc-p">
      Not a ladder gate either — a configuration choice, measured because it is the
      largest single lever in the unit.
    </p>
    ...
    <Callout kind="rule" title="Three reporting rules, all of which were learned the expensive way">
      <p>
        <b>Name the top.</b> A ladder whose top is one submodule <i>cannot see a path
        that leaves it</i>. Synthesising <code>kht_core</code> for the first time
        found it at <b>71.7 MHz</b> while the <code>kht_unit</code> inside it closed
        at 324 — the cross-lane reduction was a serial chain, 44 logic levels, and it
        lives in <code>kht_core</code> where no ladder row ever looked. Synthesising
        the assembled <code>kht_pe</code> for the first time found <b>182 MHz</b>. A
        <code>kht_unit</code> figure is not a frequency claim for this PE, including
        the reassuring ones.
      </p>
      <p>
        <b>Name which G0.</b> Today's G0 is <code>G0(int)</code> — an integer-only
        lane array. An earlier revision claimed G0 was measured against a float lane
        tier; it was not, and that sentence is what let an integer-only figure be read
        against a float-capable budget. <b>A G8 total quoted without naming its lane
        array is not a result.</b>
      </p>
      <p>
        <b>Synth is not route.</b> Every figure on this page is out-of-context
        synthesis. This project has measured a module lose 0.740 ns between synthesis
        and routing; a small negative slack at synth is not something placement
        absorbs. These numbers size the design and rank the gates. They do not close
        timing.
      </p>
    </Callout>
    ...
    <p class="doc-p">
      The SIMT PE <b>runs shaders end to end on the machine bench</b> — through the
      real L1, the real memory agent and an AXI RAM — and the resulting DRAM matches
      the golden model exactly. <b>Divergence works on hardware</b>, not only in the
      model.
    </p>
    ...
    <Callout kind="measured" title="8 requests over 1 gather is the witness reading its pre-coalescer value">
      <p>
        The LSU serialises lanes today, so the ratio is <code>LANES</code> by
        construction. It is counted <b>now</b>, before a coalescer exists, so the
        improvement will be a measured change rather than a number that appears from
        nothing — a witness that only appears once the optimisation lands cannot show
        the optimisation working. <code>gpu_gather.s</code> is the case it will be
        judged on: lane <i>i</i> reads word 5<i>i</i>, so eight lanes fall on
        <b>five</b> distinct 32-byte lines and <code>16 request(s) over 2 gather(s)</code>
        should become <code>6 over 2</code>.
      </p>
    </Callout>
    ...
    <p class="doc-p">
      <b>G5</b>, the coalescer, and <b>G6</b>, MSHRs. G7 interleaves <i>issue</i>, but
      one instruction is in flight at a time and a miss holds the whole front end,
      so until G6 lets a stalled wave step aside, more waves buy hazard-free issue and
      nothing else. Also outstanding: the lane/interval walk sequencer that
      <code>FLANES &lt; LANES</code> and <code>ILANES ≤ LANES</code> both need, which
      is ruled to the DSP realm and instantiated here rather than forked.
    </p>
    ...
    <Callout kind="open" title="G5 needs a memory path that does not exist yet">
      <p>
        ...
        <b>32-byte line</b> granularity — but <code>rv_l1</code>'s CPU-side read port
        is <b>32 bits</b>, and that is a deliberate decision in a shared component.
        Against a 32-bit port a line-granular coalescer buys <b>nothing</b>: eight
        lanes on five lines still need eight word reads, because the cache already
        coalesces the <i>fills</i> by itself. Delivering the promised 8 → 5 needs a
        wide read path contained to this PE plus a per-lane word crossbar — and that
        crossbar <b>is</b> G5's headline number, which is precisely why it must stay
        out of the shared L1.
      </p>
    </Callout>
    ...
    <Callout kind="note" title="Where to go next">
      <p>
        ...
        <RouterLink to="/mpe/simt/microarchitecture" class="doc-link">the
        microarchitecture page</RouterLink> first — several of its shapes exist to
        avoid a specific failure that has already happened once. If you want to know
        what these numbers are <i>worth</i>, see
        <RouterLink to="/mpe/simt/comparison" class="doc-link">where this lands against
        shipped GPUs</RouterLink>.
      </p>
    </Callout>
  </DocPage>
</template>
