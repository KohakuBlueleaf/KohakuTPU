...
const shipped = {
  ...
  rows: [
    { p: "Mali-G77, G57, G78 — Valhall gen 1–2", f: "<b>16</b> <span class='opacity-60'>one 16-wide execution engine</span>" },
...
const meshFig = {
  nodes: [
    { id: "simd", x: 0, y: 0, w: 14, label: "8 × SIMD PE", sub: "4 float lanes each → 32" },
    { id: "simt", x: 0, y: 5, w: 14, label: "4 × SIMT PE", sub: "8 float lanes each → 32", accent: true },
    { id: "ctl", x: 0, y: 10, w: 14, label: "2 × controller PE", sub: "no float lanes" },
    { id: "sum", x: 20, y: 4.4, w: 14, label: "one mesh", sub: "64 FP FMA / clock", accent: true },
    {
      id: "g610",
      x: 38,
      y: 4.4,
...
const meshFit = {
  ...
  rows: [
    { w: "SIMD PE, SIMD 8 + 4 float", n: "8", each: "13,772", lut: "110,176", dsp: "576", bram: "104", fma: "32" },
    { w: "SIMT PE, 8 int + 8 float", n: "4", each: "21,586", lut: "86,344", dsp: "192", bram: "122", fma: "32" },
    { w: "controller PE", n: "2", each: "2,477", lut: "4,954", dsp: "0", bram: "10", fma: "—" },
    {
      w: "<b>mesh total</b>",
      n: "<b>14</b>",
      each: "",
    ...
    { w: "against the budget", n: "", each: "", lut: "~350,000", dsp: "3,072", bram: "672", fma: "—" },
    { w: "<b>used</b>", n: "", each: "", lut: "<b>58 %</b>", dsp: "<b>25 %</b>", bram: "<b>35 %</b>", fma: "" },
  ],
}

const occupancy = [
  { label: "LUT — of the ~350,000 a mesh has once fabric and memory agent are paid for", value: 58, note: "%", tone: "accent" },
...
const lands = {
  ...
  rows: [
    { w: "<b>one of our meshes</b>", f: "64", c: "350 MHz", g: "<b>44.8</b>", _tone: "good" },
    ...
    { w: "<b>our whole device</b>", f: "256", c: "350 MHz", g: "<b>179.2</b>", _tone: "good" },
    { w: "Mali-G57 MC3 — Dimensity 700 class", f: "48", c: "~950 MHz", g: "~91" },
    { w: "Adreno 540 — Snapdragon 835, 2017", f: "not disclosed", c: "~710 MHz", g: "~567" },
...
const gflops = [
  ...
  { label: "one of our meshes — 64 FMA/clk at 350 MHz", value: 44.8, tone: "accent" },
...
const frames = {
  ...
  rows: [
    { t: "720p, 30 fps, 2× overdraw", f: "55.3 M", i: "50", u: "<b>6.2 %</b>", _tone: "good" },
    { t: "1080p, 60 fps, 2× overdraw", f: "248.8 M", i: "200", u: "<b>111 %</b>", _tone: "bad" },
  ],
}
...
<template>
  ...
  >
    ...
    <p class="doc-p">
      Peak floating-point rate is <code>lanes × 2 × clock</code>, because a fused
      multiply-add is two operations. Comparing lane counts across vendors is
      meaningful; comparing GFLOPS across vendors and clock domains hides where the
      difference comes from, which for an FPGA is always the clock.
    </p>
    ...
    <Callout kind="trap" title="Superseded figures that appear in older revisions">
      <p>
        All at the <b>3.333 or 2.500 ns</b> ask and none of them this machine:
        <code>kht_core</code> at 9,653 LUT / 279.5 MHz; the integer-only assembled PE
        at 15,638 LUT / 392.8 MHz; the float-without-multiplier build at 19,215 LUT /
        405.2 MHz; and the ~22k LUT <i>estimate</i> for a float-capable PE that this
        measurement replaced. <b>Quote none of them without its ask.</b>
      </p>
    </Callout>

    <Callout kind="note" title="The float tier takes both operand widths, and that is not a build option">
      <p>
        Nothing in the table above is a dtype configuration. Operand width is selected
        <b>per instruction</b>, by a funct7 bit that reaches the lane as a port rather
        than a parameter — so there is no build of this PE that has the float tier and
        refuses one of the two widths. See
    ...
    <Callout kind="rule" title="Every rate below is computed at 350 MHz, the ask — not at either Fmax">
      <p>
        Both PE classes close <b>above</b> the 350 MHz ask in synthesis — 365.6 and
        353.4 — so the mesh clock is set by the slower of the two rather than by either
        missing. But out-of-context synthesis is not routing, and this project has
        measured a module lose <b>0.740 ns</b> between the two.
      </p>
    </Callout>

    <h2 class="doc-h2">Where that lands</h2>

    <SpecTable :cols="lands.cols" :rows="lands.rows" caption="All rows at 350 MHz for this machine." />
    ...
    <Callout kind="measured" title="Three readings">
      <p>
        <b>One mesh is four G57 cores wide and about 1.5× one in throughput.</b> Once
        the width ratio reaches 4:1 the clock deficit stops deciding the outcome.
      </p>
      <p>
        <b>One mesh matches one G610/G710 core in width exactly</b>, and reaches
        <b>41 %</b> of it in throughput. That is the standing target, and the remaining
        gap is entirely clock — 350 MHz against ~850 — not lanes.
      </p>
      <p>
        <b>The device sits between a 2020 mid-range mobile GPU and a 2017 flagship</b>
        — almost exactly 2× a Mali-G57 MC3, and 32 % of an Adreno 540. A full G610 MC6
        is not reachable: it is 1.5× the width <i>and</i> 2.4× the clock.
      </p>
    </Callout>
    ...
    <SpecTable :cols="frames.cols" :rows="frames.rows" caption="Two frame budgets at 2× overdraw, against that issue capacity." />
    ...
    <Callout kind="measured" title="1080p60 with 200-instruction shaders is now just PAST the ceiling rather than just under it">
      <p>
        111 % of issue, where the 380 MHz working assumption this page previously
        carried put it at 102 %. The measured clock moved the crossing point, and the
        honest form of the claim is the instruction budget: <b>180 per fragment at
        1080p60, 810 at 720p30</b>.
      </p>
      <p>
      ...
      <p>
        <b>So a small real-time renderer at 720p30 is comfortably inside the
    ...
    <p class="doc-p">
      Everything above prices <b>arithmetic</b>. Rendering is not arithmetic-bound on
      this machine, and the gap is fixed function. Every item below was re-checked
    ...
    <Callout kind="note" title="Precision is NOT on that list">
      <p>
        E8M15 carries FP32's full 8-bit exponent, so range is FP32-equivalent and only
        the significand is short — <b>1.5e-5 relative error</b>, which is 32× better
        than the fp16 that mobile fragment shaders actually run at, and more range than
        the FP24 that shipped in DX9-era hardware. For colour, filter weights and
        interpolation this is above the bar rather than below it.
      </p>
      <p>
        The only mobile advantage is a <i>rate</i> one: those parts run fp16 at double
        their FP32 rate, bought by dropping to E5M10.
      </p>
      <p>
        <b>Integer multiply is no longer on the list either.</b> RV32M
        <code>mul</code>/<code>mulh</code>/<code>mulhsu</code>/<code>mulhu</code> are
        built, one 33×33 signed product per lane, so <code>y * width + x</code> is one
        instruction rather than a shift-add chain. Divide and remainder still fault,
        deliberately — divide-by-a-constant strength-reduces to <code>mulhu</code>.
      </p>
    </Callout>
    ...
    <Callout kind="open" title="Until item 1 lands, treat the frame budgets as an upper bound on what the arithmetic permits, not a frame rate">
      <p>
        Nothing on this page prices triangle setup, coverage, or the driver that would
        have to produce the shader in the first place.
      </p>
    </Callout>
  </DocPage>
</template>
