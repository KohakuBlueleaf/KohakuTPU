...
const totals = [
  ...
  {
    label: "station line — what is built",
    value: 22106,
    note: "one synthesis",
    tone: "good",
  },
...
const chooseRows = [
  {
    knob: "FW",
    val: "256",
    why: "the widest slave, <code>S_AXI_MEM</code>, is 256 bits, so the fabric meets it exactly and the 512-bit XDMA master splits 2:1. 22,106 LUTs against 30,785 at 512 — 28% — and 629 cross-SLR wires against 1,173. At 200 MHz it carries 6.4 GB/s, exactly what the 512-bit-at-100 MHz SmartConnect provided",
  },
  {
    knob: "AW",
    val: "43",
    why: "forced: mesh MEM sits at <code>(id+1) &lt;&lt; 40</code>. Costs 3.6% over 32 bits (21,345 → 22,106)",
  },
  {
    knob: "bus clock",
    val: "200 MHz",
...
<template>
  ...
  >
    <p class="doc-p">
      ...
      cache and protection attributes.
    </p>
    ...
    <p class="doc-p">
      ...
      inject mesh traffic with an ordinary AXI write and nothing else.
    </p>
    ...
    <Fig
      caption="There is no crossbar, because there is one slave: arbitration is a mux and response routing is a decode of bits that are already in flight — BID and RID say where each response goes, so no scoreboard is kept and none has to be sized."
      zoom
      wide
    >
    ...
    <Callout
      kind="rule"
      title="One run of the machine should be one host transaction"
    >
      <p>
        The host is not in the loop per poll, and the same program works over a
        debug bridge and over a production DMA path — which matters more than it
        sounds, because
        <b
          >a single debug read can cost milliseconds against microseconds of
          compute</b
        >. It is not a fabric node: its reach into the mesh is an AXI write into
        a control window, which the control agent turns into a flit, so
        dispatch, configuration and debug injection all share one mechanism.
      </p>
    </Callout>
    ...
    <p class="doc-p">
      ...
      span multiple clock domains, data widths and dies.
    </p>
    ...
    <Callout kind="note" title="A tree pays AXI ports to reach itself">
      ...
      <p>
        ...
        <b>9,795 LUTs against 1,641</b>, a factor of 6.0.
      </p>
    </Callout>
    ...
    >
      ...
      <p>
        ...
        as <code>mag_link_cdc</code> does — <i>can</i> overflow, and then a
        sticky fault bit and a software poll are load bearing. That is a
        property of that flow-control choice, not of clock crossing.
      </p>
    </Callout>
    ...
    <Callout kind="rule" title="The case for it is heterogeneity, not size">
      <p>
        ...
        (1.15×), while the four-die replacement wins 3.70× overall and 5.97× on
      ...
      <p>
        ...
        pipelines are flops by design — six link instances carry 12,166 of the
        deployed line's 48,167 flip-flops.
      </p>
    </Callout>
