...
const TONE = { good: "bg-sage", bad: "bg-coral", warn: "bg-amber", accent: "bg-gem" }
</script>
  ...
  <div class="my-5 doc-w card p-4">
    <div v-for="it in props.items" :key="it.label" class="mb-3 last:mb-0">
      <div class="flex items-baseline justify-between gap-3 mb-1">
        <span class="kt-text-caption text-warm-700 dark:text-warm-300">{{ it.label }}</span>
        <span class="kt-text-micro font-mono text-warm-500 dark:text-warm-400 tabular-nums">
          {{ fmt(it.value) }}<span v-if="it.note" class="ml-2 opacity-70">{{ it.note }}</span>
        </span>
      </div>
      <div class="h-2 rounded-full bg-warm-100 dark:bg-warm-800 overflow-hidden">
        <div
    ...
    <p v-if="props.caption" class="kt-text-caption text-warm-500 dark:text-warm-400 mt-3">
