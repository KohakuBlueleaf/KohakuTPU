...
const U = props.unit
const P = computed(() =>
  Object.fromEntries(props.states.map((s) => [s.id, { x: s.x * U, y: s.y * U }])),
)
...
<template>
  <svg :viewBox="vb" class="dgm" role="img">
    <defs>
      <marker id="sm-arrow" viewBox="0 0 8 8" refX="7" refY="4" markerWidth="6"
    ...
    <g v-for="(e, i) in props.edges" :key="i">
      ...
      <text v-if="e.label" :x="lab(e).x" :y="lab(e).y" text-anchor="middle" class="dgm-sub">
        {{ e.label }}
      </text>
    </g>
    ...
    <g v-for="s in props.states" :key="s.id">
      <circle
        :cx="P[s.id].x" :cy="P[s.id].y" :r="props.r"
        :class="s.accent || props.active === s.id ? 'dgm-box-accent' : 'dgm-box'"
      />
      <text
        :x="P[s.id].x" :y="P[s.id].y + (s.sub ? -1 : 4)"
        text-anchor="middle" class="dgm-label" font-weight="600"
      >{{ s.label }}</text>
      <text
        v-if="s.sub" :x="P[s.id].x" :y="P[s.id].y + 10"
        text-anchor="middle" class="dgm-sub"
      >{{ s.sub }}</text>
    </g>
  </svg>
</template>
