// SUPERSEDED by sb_stn_line.v: NK link ports is a hub-and-spoke, the shape this
// design exists to replace. A station has a left and a right neighbour.

// A root station: NM managers, NL local subordinates, NK links to other
// stations. sb_slr1 without link ports was a lower bound; this is the real one.

// REQ out is ONE broadcast payload with NK valids, not NK payloads -- ejection
// is a demux, so a second link costs a valid and a ready, not a second bus.

`default_nettype none

module sb_stn_root #(
    parameter integer FW    = 512,
    parameter integer AW    = 40,
    parameter integer MAXW  = 512,
    parameter integer MAXID = 4,
    parameter integer NM    = 3,
    parameter integer NL    = 2,                // local subordinates
    parameter integer NK    = 3,                // links to other stations
    parameter integer NS    = NL + NK,
    parameter integer TAGW  = 4,
    parameter integer DSTW  = 3,
    parameter integer SRCW  = 2,
    parameter integer OST   = 4,
    parameter integer STORE_FWD    = 1,
    parameter integer LUT_PER_BRAM = 0,
    parameter integer TIMEOUT      = 0,
    parameter integer STATS        = 0,
    // Bit i set makes local subordinate i FW wide, clear makes it 32.
    parameter [31:0]  LOC_W = 32'h0000_0001,
    // One shared map for every manager: the validation build wants any manager
    // able to reach any endpoint, which per-NMU tables cannot express.
    parameter integer NSEG = 1,
    parameter [NSEG*AW-1:0]   SEG_BASE  = {NSEG{{AW{1'b0}}}},
    parameter [NSEG*AW-1:0]   SEG_MASK  = {NSEG{{AW{1'b0}}}},
    parameter [NSEG*DSTW-1:0] SEG_DST   = {NSEG{{DSTW{1'b0}}}},
    parameter [NSEG*DSTW-1:0] SEG_DPORT = {NSEG{{DSTW{1'b0}}}}
)(
    input  wire                 bus_clk,
    input  wire                 bus_rst,
    input  wire                 clk_ctrl,   input wire aresetn_ctrl,
    input  wire                 clk_xdma,   input wire aresetn_xdma,
    input  wire                 clk_loc,    input wire aresetn_loc,

    input  wire [NM*MAXID-1:0]    mp_awid,
    input  wire [NM*AW-1:0]       mp_awaddr,
    input  wire [NM*8-1:0]        mp_awlen,
    input  wire [NM*3-1:0]        mp_awsize,
    input  wire [NM*2-1:0]        mp_awburst,
    input  wire [NM-1:0]          mp_awvalid,
    output wire [NM-1:0]          mp_awready,
    input  wire [NM*MAXW-1:0]     mp_wdata,
    input  wire [NM*(MAXW/8)-1:0] mp_wstrb,
    input  wire [NM-1:0]          mp_wlast,
    input  wire [NM-1:0]          mp_wvalid,
    output wire [NM-1:0]          mp_wready,
    output wire [NM*MAXID-1:0]    mp_bid,
    output wire [NM*2-1:0]        mp_bresp,
    output wire [NM-1:0]          mp_bvalid,
    input  wire [NM-1:0]          mp_bready,
    input  wire [NM*MAXID-1:0]    mp_arid,
    input  wire [NM*AW-1:0]       mp_araddr,
    input  wire [NM*8-1:0]        mp_arlen,
    input  wire [NM*3-1:0]        mp_arsize,
    input  wire [NM*2-1:0]        mp_arburst,
    input  wire [NM-1:0]          mp_arvalid,
    output wire [NM-1:0]          mp_arready,
    output wire [NM*MAXID-1:0]    mp_rid,
    output wire [NM*MAXW-1:0]     mp_rdata,
    output wire [NM*2-1:0]        mp_rresp,
    output wire [NM-1:0]          mp_rlast,
    output wire [NM-1:0]          mp_rvalid,
    input  wire [NM-1:0]          mp_rready,

    output wire [NL*MAXID-1:0]    sp_awid,
    output wire [NL*AW-1:0]       sp_awaddr,
    output wire [NL*8-1:0]        sp_awlen,
    output wire [NL*3-1:0]        sp_awsize,
    output wire [NL*2-1:0]        sp_awburst,
    output wire [NL-1:0]          sp_awvalid,
    input  wire [NL-1:0]          sp_awready,
    output wire [NL*MAXW-1:0]     sp_wdata,
    output wire [NL*(MAXW/8)-1:0] sp_wstrb,
    output wire [NL-1:0]          sp_wlast,
    output wire [NL-1:0]          sp_wvalid,
    input  wire [NL-1:0]          sp_wready,
    input  wire [NL*MAXID-1:0]    sp_bid,
    input  wire [NL*2-1:0]        sp_bresp,
    input  wire [NL-1:0]          sp_bvalid,
    output wire [NL-1:0]          sp_bready,
    output wire [NL*MAXID-1:0]    sp_arid,
    output wire [NL*AW-1:0]       sp_araddr,
    output wire [NL*8-1:0]        sp_arlen,
    output wire [NL*3-1:0]        sp_arsize,
    output wire [NL*2-1:0]        sp_arburst,
    output wire [NL-1:0]          sp_arvalid,
    input  wire [NL-1:0]          sp_arready,
    input  wire [NL*MAXID-1:0]    sp_rid,
    input  wire [NL*MAXW-1:0]     sp_rdata,
    input  wire [NL*2-1:0]        sp_rresp,
    input  wire [NL-1:0]          sp_rlast,
    input  wire [NL-1:0]          sp_rvalid,
    output wire [NL-1:0]          sp_rready,

    // ---- links out: one shared payload, NK valid/ready pairs ---------------
    output wire [NK-1:0]        lk_req_valid,
    input  wire [NK-1:0]        lk_req_ready,
    output wire [DSTW-1:0]      lk_req_dport,
    output wire [SRCW-1:0]      lk_req_src,
    output wire [TAGW-1:0]      lk_req_tag,
    output wire                 lk_req_wr,
    output wire                 lk_req_head,
    output wire                 lk_req_last,
    output wire [AW-1:0]        lk_req_addr,
    output wire [7:0]           lk_req_len,
    output wire [2:0]           lk_req_size,
    output wire [FW-1:0]        lk_req_data,
    output wire [FW/8-1:0]      lk_req_strb,

    input  wire [NK-1:0]        lk_rsp_valid,
    output wire [NK-1:0]        lk_rsp_ready,
    input  wire [NK*SRCW-1:0]   lk_rsp_dst,
    input  wire [NK*TAGW-1:0]   lk_rsp_tag,
    input  wire [NK-1:0]        lk_rsp_wr,
    input  wire [NK-1:0]        lk_rsp_last,
    input  wire [NK*2-1:0]      lk_rsp_resp,
    input  wire [NK*FW-1:0]     lk_rsp_data,

    output wire [31:0]          stat_decerr,
    output wire [127:0]         stat_bus
);
    wire [NM-1:0]         q_valid, q_ready, q_wr, q_head, q_last;
    wire [NM*DSTW-1:0]    q_dst, q_dpt;
    wire [NM*TAGW-1:0]    q_tag;
    wire [NM*AW-1:0]      q_addr;
    wire [NM*8-1:0]       q_len;
    wire [NM*3-1:0]       q_size;
    wire [NM*FW-1:0]      q_data;
    wire [NM*(FW/8)-1:0]  q_strb;

    wire [NS-1:0]         e_valid, e_ready;
    wire [SRCW-1:0]       e_src;
    wire [DSTW-1:0]       e_dpt;
    wire [TAGW-1:0]       e_tag;
    wire                  e_wr, e_head, e_last;
    wire [AW-1:0]         e_addr;
    wire [7:0]            e_len;
    wire [2:0]            e_size;
    wire [FW-1:0]         e_data;
    wire [FW/8-1:0]       e_strb;

    wire [NS-1:0]         p_valid, p_ready, p_wr, p_last;
    wire [NS*SRCW-1:0]    p_dst;
    wire [NS*TAGW-1:0]    p_tag;
    wire [NS*2-1:0]       p_resp;
    wire [NS*FW-1:0]      p_data;

    wire [NM-1:0]         d_valid, d_ready;
    wire [TAGW-1:0]       d_tag;
    wire                  d_wr, d_last;
    wire [1:0]            d_resp;
    wire [FW-1:0]         d_data;

    sb_station #(.NM(NM), .NS(NS), .FW(FW), .AW(AW), .TAGW(TAGW),
                 .DSTW(DSTW), .SRCW(SRCW), .STATS(STATS)) u_stn (
        .stat_req_flits(stat_bus[31:0]),   .stat_req_wait(stat_bus[63:32]),
        .stat_rsp_flits(stat_bus[95:64]),  .stat_rsp_wait(stat_bus[127:96]),
        .clk(bus_clk), .rst(bus_rst),
        .nm_req_valid(q_valid), .nm_req_ready(q_ready), .nm_req_dst(q_dst),
        .nm_req_dport(q_dpt), .nm_req_src({NM*SRCW{1'b0}}),
        .nm_req_tag(q_tag), .nm_req_wr(q_wr), .nm_req_head(q_head),
        .nm_req_last(q_last), .nm_req_addr(q_addr), .nm_req_len(q_len),
        .nm_req_size(q_size), .nm_req_data(q_data), .nm_req_strb(q_strb),
        .ns_req_valid(e_valid), .ns_req_ready(e_ready), .ns_req_src(e_src),
        .ns_req_dport(e_dpt), .ns_req_tag(e_tag), .ns_req_wr(e_wr),
        .ns_req_head(e_head), .ns_req_last(e_last), .ns_req_addr(e_addr),
        .ns_req_len(e_len), .ns_req_size(e_size), .ns_req_data(e_data),
        .ns_req_strb(e_strb),
        .ns_rsp_valid(p_valid), .ns_rsp_ready(p_ready), .ns_rsp_dst(p_dst),
        .ns_rsp_tag(p_tag), .ns_rsp_wr(p_wr), .ns_rsp_last(p_last),
        .ns_rsp_resp(p_resp), .ns_rsp_data(p_data),
        .nm_rsp_valid(d_valid), .nm_rsp_ready(d_ready), .nm_rsp_dst(),
        .nm_rsp_tag(d_tag), .nm_rsp_wr(d_wr), .nm_rsp_last(d_last),
        .nm_rsp_resp(d_resp), .nm_rsp_data(d_data)
    );

    // ------------------------------------------------------------ link fan-out
    assign lk_req_dport = e_dpt;
    assign lk_req_src   = e_src;
    assign lk_req_tag   = e_tag;
    assign lk_req_wr    = e_wr;
    assign lk_req_head  = e_head;
    assign lk_req_last  = e_last;
    assign lk_req_addr  = e_addr;
    assign lk_req_len   = e_len;
    assign lk_req_size  = e_size;
    assign lk_req_data  = e_data;
    assign lk_req_strb  = e_strb;

    genvar k;
    generate
    for (k = 0; k < NK; k = k + 1) begin : g_link
        assign lk_req_valid[k]   = e_valid[NL+k];
        assign e_ready[NL+k]     = lk_req_ready[k];
        assign p_valid[NL+k]     = lk_rsp_valid[k];
        assign lk_rsp_ready[k]   = p_ready[NL+k];
        assign p_dst [(NL+k)*SRCW +: SRCW] = lk_rsp_dst[k*SRCW +: SRCW];
        assign p_tag [(NL+k)*TAGW +: TAGW] = lk_rsp_tag[k*TAGW +: TAGW];
        assign p_wr  [NL+k]                = lk_rsp_wr[k];
        assign p_last[NL+k]                = lk_rsp_last[k];
        assign p_resp[(NL+k)*2 +: 2]       = lk_rsp_resp[k*2 +: 2];
        assign p_data[(NL+k)*FW +: FW]     = lk_rsp_data[k*FW +: FW];
    end
    endgenerate

    wire [31:0] dc0, dc1, dc2;
    assign stat_decerr = dc0 + dc1 + dc2;
    assign mp_rdata[0*MAXW + 32 +: MAXW-32] = {(MAXW-32){1'b0}};
    assign mp_rdata[2*MAXW + 32 +: MAXW-32] = {(MAXW-32){1'b0}};

`define RT_FLIT(I) \
        .req_valid(q_valid[I]), .req_ready(q_ready[I]), \
        .req_dst(q_dst[(I)*DSTW +: DSTW]), \
        .req_dport(q_dpt[(I)*DSTW +: DSTW]), \
        .req_tag(q_tag[(I)*TAGW +: TAGW]), \
        .req_wr(q_wr[I]), .req_head(q_head[I]), .req_last(q_last[I]), \
        .req_addr(q_addr[(I)*AW +: AW]), .req_len(q_len[(I)*8 +: 8]), \
        .req_size(q_size[(I)*3 +: 3]), .req_data(q_data[(I)*FW +: FW]), \
        .req_strb(q_strb[(I)*(FW/8) +: FW/8]), \
        .rsp_valid(d_valid[I]), .rsp_ready(d_ready[I]), .rsp_tag(d_tag), \
        .rsp_wr(d_wr), .rsp_last(d_last), .rsp_resp(d_resp), .rsp_data(d_data)

`define RT_AXI(I, W) \
        .s_awid(mp_awid[(I)*MAXID +: MAXID]), \
        .s_awaddr(mp_awaddr[(I)*AW +: AW]), \
        .s_awlen(mp_awlen[(I)*8 +: 8]), .s_awsize(mp_awsize[(I)*3 +: 3]), \
        .s_awburst(mp_awburst[(I)*2 +: 2]), .s_awvalid(mp_awvalid[I]), \
        .s_awready(mp_awready[I]), \
        .s_wdata(mp_wdata[(I)*MAXW +: W]), \
        .s_wstrb(mp_wstrb[(I)*(MAXW/8) +: (W)/8]), \
        .s_wlast(mp_wlast[I]), .s_wvalid(mp_wvalid[I]), \
        .s_wready(mp_wready[I]), \
        .s_bid(mp_bid[(I)*MAXID +: MAXID]), .s_bresp(mp_bresp[(I)*2 +: 2]), \
        .s_bvalid(mp_bvalid[I]), .s_bready(mp_bready[I]), \
        .s_arid(mp_arid[(I)*MAXID +: MAXID]), \
        .s_araddr(mp_araddr[(I)*AW +: AW]), \
        .s_arlen(mp_arlen[(I)*8 +: 8]), .s_arsize(mp_arsize[(I)*3 +: 3]), \
        .s_arburst(mp_arburst[(I)*2 +: 2]), .s_arvalid(mp_arvalid[I]), \
        .s_arready(mp_arready[I]), \
        .s_rid(mp_rid[(I)*MAXID +: MAXID]), \
        .s_rdata(mp_rdata[(I)*MAXW +: W]), .s_rresp(mp_rresp[(I)*2 +: 2]), \
        .s_rlast(mp_rlast[I]), .s_rvalid(mp_rvalid[I]), .s_rready(mp_rready[I])

`define RT_NMU(W, DEP, RDEP) \
        .MW(W), .MIDW(MAXID), .AW(AW), .FW(FW), .TAGW(TAGW), .DSTW(DSTW), \
        .LUT_PER_BRAM(LUT_PER_BRAM), .STORE_FWD(STORE_FWD), \
        .NSEG(NSEG), .REQ_DEPTH(DEP), .RSP_DEPTH(RDEP), \
        .SEG_BASE(SEG_BASE), .SEG_MASK(SEG_MASK), .SEG_XLT(SEG_BASE), \
        .SEG_DST(SEG_DST), .SEG_DPORT(SEG_DPORT), .SEG_VLD({NSEG{1'b1}})

    sb_nmu #(`RT_NMU(32, 16, 16)) u_nmu0 (
        .s_aclk(clk_ctrl), .s_aresetn(aresetn_ctrl),
        `RT_AXI(0, 32), .bus_clk(bus_clk), .bus_rst(bus_rst),
        `RT_FLIT(0), .stat_decerr(dc0));

    sb_nmu #(`RT_NMU(512, 64, 64)) u_nmu1 (
        .s_aclk(clk_xdma), .s_aresetn(aresetn_xdma),
        `RT_AXI(1, 512), .bus_clk(bus_clk), .bus_rst(bus_rst),
        `RT_FLIT(1), .stat_decerr(dc1));

    sb_nmu #(`RT_NMU(32, 16, 16)) u_nmu2 (
        .s_aclk(clk_xdma), .s_aresetn(aresetn_xdma),
        `RT_AXI(2, 32), .bus_clk(bus_clk), .bus_rst(bus_rst),
        `RT_FLIT(2), .stat_decerr(dc2));

    genvar i;
    generate
    for (i = 0; i < NL; i = i + 1) begin : g_nsu
        localparam integer DW = LOC_W[i] ? FW : 32;
        if (DW < MAXW) begin : g_pad
            assign sp_wdata[i*MAXW + DW +: MAXW-DW] = {(MAXW-DW){1'b0}};
            assign sp_wstrb[i*(MAXW/8) + DW/8 +: (MAXW-DW)/8] =
                   {((MAXW-DW)/8){1'b0}};
        end
        sb_nsu #(.SDW(DW), .SIDW(MAXID), .AW(AW), .FW(FW), .TAGW(TAGW),
                 .SRCW(SRCW), .WOST(OST), .ROST(OST), .TIMEOUT(TIMEOUT),
                 .LUT_PER_BRAM(LUT_PER_BRAM),
                 .REQ_DEPTH(16), .RSP_DEPTH(16)) u_nsu (
            .bus_clk(bus_clk), .bus_rst(bus_rst),
            .req_valid(e_valid[i]), .req_ready(e_ready[i]), .req_src(e_src),
            .req_tag(e_tag), .req_wr(e_wr), .req_head(e_head),
            .req_last(e_last), .req_addr(e_addr), .req_len(e_len),
            .req_size(e_size), .req_data(e_data), .req_strb(e_strb),
            .rsp_valid(p_valid[i]), .rsp_ready(p_ready[i]),
            .rsp_dst(p_dst[i*SRCW +: SRCW]), .rsp_tag(p_tag[i*TAGW +: TAGW]),
            .rsp_wr(p_wr[i]), .rsp_last(p_last[i]),
            .rsp_resp(p_resp[i*2 +: 2]), .rsp_data(p_data[i*FW +: FW]),
            .m_aclk(clk_loc), .m_aresetn(aresetn_loc),
            .m_awid(sp_awid[i*MAXID +: MAXID]),
            .m_awaddr(sp_awaddr[i*AW +: AW]), .m_awlen(sp_awlen[i*8 +: 8]),
            .m_awsize(sp_awsize[i*3 +: 3]), .m_awburst(sp_awburst[i*2 +: 2]),
            .m_awvalid(sp_awvalid[i]), .m_awready(sp_awready[i]),
            .m_wdata(sp_wdata[i*MAXW +: DW]),
            .m_wstrb(sp_wstrb[i*(MAXW/8) +: DW/8]),
            .m_wlast(sp_wlast[i]), .m_wvalid(sp_wvalid[i]),
            .m_wready(sp_wready[i]),
            .m_bid(sp_bid[i*MAXID +: MAXID]), .m_bresp(sp_bresp[i*2 +: 2]),
            .m_bvalid(sp_bvalid[i]), .m_bready(sp_bready[i]),
            .m_arid(sp_arid[i*MAXID +: MAXID]),
            .m_araddr(sp_araddr[i*AW +: AW]), .m_arlen(sp_arlen[i*8 +: 8]),
            .m_arsize(sp_arsize[i*3 +: 3]), .m_arburst(sp_arburst[i*2 +: 2]),
            .m_arvalid(sp_arvalid[i]), .m_arready(sp_arready[i]),
            .m_rid(sp_rid[i*MAXID +: MAXID]),
            .m_rdata(sp_rdata[i*MAXW +: DW]), .m_rresp(sp_rresp[i*2 +: 2]),
            .m_rlast(sp_rlast[i]), .m_rvalid(sp_rvalid[i]),
            .m_rready(sp_rready[i])
        );
    end
    endgenerate
endmodule

`default_nettype wire
