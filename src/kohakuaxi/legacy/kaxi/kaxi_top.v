// KohakuAXI full system: M x N same-width crossbar with a per-home L3 cache.
//   M master slave ports  ->  kaxi_xbar  ->  N x kaxi_l3  ->  N MIG master ports
// The crossbar routes on address; each home has its own cache (memory-side, so
// coherence-free -- one home owns each address); a hit never reaches the MIG.
// Single clock in this milestone (per-port clocks / CDC and multi-SLR sharding
// come next). L3_EN lets a home be a bypass (no cache) via a passthrough.
...
module kaxi_top #(
    ...
    parameter integer L3_SETS  = 256,
    parameter integer L3_SET_W = 8,
    parameter integer L3_LSB   = 5,
    ...
    // Crossbar home ports (between the xbar and the per-home caches).
    ...
    genvar h;
    generate
    for (h = 0; h < N_HOME; h = h + 1) begin : g_l3
        kaxi_l3 #(.ADDR_W(ADDR_W), .DATA_W(DATA_W), .ID_W(SID_W),
                  .SETS(L3_SETS), .LINE_LSB(L3_LSB), .SET_W(L3_SET_W)) u_l3 (
            .clk(clk), .resetn(resetn),
