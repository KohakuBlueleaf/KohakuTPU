...
module mx_acu_fp_pump #(
    ...
    always @(posedge clk) begin
        ...
        // them qualified downstream by cmdv_q.
        if (rst) begin
            op_q <= OP_NOP; cmdv_q <= 1'b0;
    ...
    reg         sing_q;
    always @(posedge clk) begin
        // NO RESET: the odd half of the same command, qualified by the same
        // cmdv_q, and `sing_q` beside it was already unreset.
        if (en) begin
    ...
    for (gs = 0; gs < 4; gs = gs + 1) begin : g_mrg_r
        for (gt = 0; gt < 4; gt = gt + 1) begin : g_mrg_c
            ...
            wire big2 = d[10];
            wire [VWM-1:0] m0 = lm_a[L];
            wire [VWM-1:0] m1 = lm_b[L];
            wire           g0 = ls_a[L];
            wire           g1 = ls_b[L];
    ...
    integer i, j;
    always @(posedge clk) begin
        // 16 lanes of val/sgn/exp plus a TW peer copy, all gated by v1.
        if (rst) begin
            v1 <= 1'b0; op_r <= OP_NOP;
        end else if (en) begin
            v1     <= cmdv_d2;
    ...
    integer q;
    always @(posedge clk) begin
        // Same shape one stage on, gated by v1_s.
        if (rst) begin
            v1_s <= 1'b0; op_s <= OP_NOP;
        end else if (en) begin
    ...
    always @(posedge clk) begin
        ...
        // are 2*TW+TAW of payload that v2 gates.
        if (rst) begin
...
`endif
    ...
    always @(posedge clk) begin
        // v3 gates the write, the output select and all 16 lanes below it, so it
        // is the only bit here that has to survive reset.
        if (rst) begin
            v3 <= 1'b0;
        end else if (en) begin
    ...
    always @(posedge clk) begin
        // Same as stage 3: the tile write is `en && v4 && s4_wr`, so v4 alone.
        if (rst) begin
            v4 <= 1'b0;
        end else if (en) begin
            v4       <= v3;
    ...
    reg [3:0] busy_tail;
    // cmdv_q covers stage 0 and cmdv_d1/d2 the merge's two, or busy drops
    // mid-command and the DRAIN reads a half-written tile.
