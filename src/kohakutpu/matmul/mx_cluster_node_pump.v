...
// DOUBLE PUMPED. L1 and the cascade run on clk2x and produce two partials per
// clk cycle; the accumulator takes both and still writes the tile once.
module mx_cluster_node_pump #(
    ...
    // Ports are clk1x/clk2x, not clk, because ooc_pump.tcl constrains by name.
    input  wire         clk1x,
    input  wire         clk2x,
    input  wire         rst,
    ...
    // Everything below stays on `clk`; only u_core and the L1 half of u_mgr move.
    wire clk = clk1x;
    ...
    wire [TAW-1:0]  m_addr;
    wire            m_cmd;
    wire [31:0]     m_sa, m_sb, m_sa2, m_sb2;
    wire            m_single;
    wire [7:0]      m_anchor;
    ...
    // REGISTERED: combinational into mx_cluster_cu's FSM enable these are a
    // 2x -> 1x path of 1.781 ns, capping clk2x at 561 MHz.
    wire mgr_busy, acu_busy;
    reg  gemm_busy_r, sweep_busy_r, start_r;
    always @(posedge clk) begin
        if (rst) begin
            gemm_busy_r <= 1'b0; sweep_busy_r <= 1'b0; start_r <= 1'b0;
        end else begin
            gemm_busy_r  <= mgr_busy || acu_busy;
            sweep_busy_r <= mgr_busy;
            start_r      <= gemm_start;
        end
    end
    // `start_r` covers the cycle the register costs on the RISE. Without it
    // busy reads low for one cycle after gemm_start and a DRAIN can slip in.
    assign gemm_busy  = gemm_busy_r  || start_r;
    assign sweep_busy = sweep_busy_r || start_r;
    ...
    mx_cluster_mgr_pump #(.GA(GA), .GB(GB), .TAW(TAW), .L1_PRIM(L1_PRIM)) u_mgr (
        ...
        .acu_sa(m_sa), .acu_sb(m_sb),
        .acu_sa2(m_sa2), .acu_sb2(m_sb2), .acu_single(m_single),
        .acu_anchor(m_anchor)
    );
    ...
    wire [383:0] core_part;
    wire         core_pv, core_pf;

    mx_cluster_core #(.MODEL(MODEL)) u_core (
        .clk(clk2x), .rst(rst), .en(1'b1),
        .a_in(a_bus), .b_in(b_bus),
        .in_valid(core_valid), .in_first(core_first),
        .part_out(core_part), .part_valid(core_pv), .part_first(core_pf)
    );

    // ---- the pair, 2x -> 1x ----------------------------------------------

    // The two partials are consecutive K blocks of the SAME sub-tile, but they
    // carry DIFFERENT E5M3 scales, so they cannot be summed packed here.
    reg          pp;
    reg [383:0]  p_lo, p_lo_h, p_hi;
    reg          pair_v, pair_f;

    // `pp` follows VALID, not the clock phase.
    always @(posedge clk2x) begin
        // `pp` is the pair phase and must survive reset; p_lo is 384 bits of
        // half-pair that `pair_v` gates, as p_lo_h beside it already shows.
        if (rst) begin
            pp <= 1'b0; pair_v <= 1'b0; pair_f <= 1'b0;
        end else begin
            pair_v <= 1'b0;
            if (core_pv) begin
                pp <= ~pp;
                if (!pp) begin
                    ...
                    p_lo_h <= p_lo;        // both halves latched together
                    p_hi   <= core_part;
                    pair_v <= 1'b1;
                end
            end
    ...
    // Two clk2x cycles wide so the clk edge cannot miss it. NOT replaceable by a
    // toggle handshake: part_bus is read WITH part_valid, so sync latency breaks.
    reg pair_v2;
    always @(posedge clk2x) pair_v2 <= (rst ? 1'b0 : pair_v);

    assign part_bus   = p_hi;
    assign part_valid = pair_v | pair_v2;
    assign part_first = pair_f;

    wire [383:0] part_bus2 = p_lo_h;
    ...
    always @(posedge clk) begin
        if (rst) begin
            pq_pend <= 1'b0;    // pq_hi is PW-256 bits that pq_pend qualifies
        end else begin
            pq_pend <= acu_peer_v;
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            // `cmd_valid` -- and d_peer alone is PW flops.
        end else begin
            d_cmd <= 1'b0;
...
`ifndef SYNTHESIS
    ...
    always @(posedge clk) begin
        if (!rst && dq_wr && dq_full)
            $display("%0t ERROR mx_cluster_node_pump: drain queue overflow, sub-tile lost",
                     $time);
        ...
        if (!rst && acu_peer_v && pq_pend)
            $display("%0t ERROR mx_cluster_node_pump: peer results collided, high half lost",
                     $time);
    end
`endif
    ...
    mx_acu_fp_pump #(.DEPTH(TILES), .ACC_MW(ACC_MW), .TILE_PRIM(TILE_PRIM)) u_acu (
        .clk(clk), .rst(rst), .en(1'b1),
        .part_in(part_bus),
        .part_in2(part_bus2), .sa2(m_sa2), .sb2(m_sb2), .single(m_single),
