...
module mx_cluster_node #(
    ...
    mx_cluster_mgr #(.GA(GA), .GB(GB), .TAW(TAW), .L1_PRIM(L1_PRIM),
                     .PUMP(PUMP), .PH_INV(PH_INV)) u_mgr (
    ...
    wire [383:0] p_sum;
    genvar gi;
    generate for (gi = 0; gi < 8; gi = gi + 1) begin : g_psum
        assign p_sum[gi*48 +: 48] = p_lo[gi*48 +: 48] + core_part[gi*48 +: 48];
    end endgenerate

    always @(posedge clk2x) begin
        if (rst) begin
            pp <= 1'b0; pair_v <= 1'b0; pair_f <= 1'b0;
            p_lo <= 384'd0; pair_d <= 384'd0;
        end else begin
            pair_v <= 1'b0;
            if (core_pv) begin
                pp <= ~pp;
    ...
    // PUMP=1 (fix B) sums the pair packed and hands the ACU one partial.
    // PUMP=2 (fix A) hands it BOTH, each with its own scale.
    ...
    assign part_first = (PUMP != 0) ? pair_f : core_pf;

    wire [383:0] part_bus2 = p_lo_h;

`ifndef SYNTHESIS
    always @(posedge clk2x) if (PUMP != 0 && !rst && core_pv && $time < 600000)
        $display("%0t 2xPART pp=%0b first=%0b p=%0d", $time, pp, core_pf,
                 $signed(core_part[18:0]));

    // Printed for PUMP=0 as well, so a single-clock run gives the reference
    // command stream to diff the pumped one against.
    always @(posedge clk) if (!rst && m_cmd)
        $display("REF a=%0d op=%0d s=%0b sa=%h sb=%h sa2=%h sb2=%h P1=%h P0=%h",
                 m_addr, m_op, m_single, m_sa, m_sb, m_sa2, m_sb2,
                 part_bus, part_bus2);
`endif
...
`endif
    ...
    mx_acu_fp #(.DEPTH(TILES), .ACC_MW(ACC_MW), .TILE_PRIM(TILE_PRIM),
                .DUAL(PUMP >= 2), .TMUX(PUMP == 3)) u_acu (
