// A leaf station: no managers of its own, NS local subordinates, one link.
// The SLR2 shape -- mesh mem, mesh ctrl, ddr ctrl, its clock wizard, the GPIO.

// With one REQ source the injection mux disappears entirely, which is why a
// leaf costs so much less than leaf_smc_2's 13,266 LUT for the same endpoints.
...
module sb_leaf #(
    ...
    // ---- from the link: one REQ stream in, one RSP stream out -------------
