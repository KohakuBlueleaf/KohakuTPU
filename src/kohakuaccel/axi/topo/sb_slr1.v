// v5's SLR1 station as it would actually be built: 3 managers, 5 LOCAL
// subordinates, 4 clocks. The two leaves are links, not master ports.

// This is why root_smc is 9 MI and a station is 5: SmartConnect can only reach
// another SLR through a master port plus an axi_register_slice.

// No link ports yet, so this is a LOWER BOUND: add sb_link when it exists, and
// xdma's table is short by the three off-station windows (three comparators).
...
module sb_slr1 #(
    ...
    // S0 mesh_1/S_AXI_MEM, S1 mesh_1/S_AXI_CTRL, S2 ddr4_3 ctrl,
    // S3 clk_wiz_mesh_1, S4 clk_wiz_ctrl.
    ...
    localparam [AW-1:0] B_M1  = 40'h90_0000_0000;
    localparam [AW-1:0] B_MC  = 40'h00_0081_0000, B_DDR = 40'h00_0030_0000;
    localparam [AW-1:0] B_W1  = 40'h00_0091_0000, B_WC  = 40'h00_0090_0000;
    ...
    localparam [NSEG1*AW-1:0]   S1_BASE = B_M1;
    localparam [NSEG1*AW-1:0]   S1_MASK = MSK_MESH;
    localparam [NSEG1*DSTW-1:0] S1_DST  = 3'd0;

    localparam integer NSEG2 = 4;
    localparam [NSEG2*AW-1:0]   S2_BASE = {B_WC, B_W1, B_DDR, B_MC};
    localparam [NSEG2*AW-1:0]   S2_MASK = {MSK_64K, MSK_64K, MSK_1M, MSK_64K};
    localparam [NSEG2*DSTW-1:0] S2_DST  = {3'd4, 3'd3, 3'd2, 3'd1};

    wire [NS-1:0] sclk  = {clk_ctrl, clk_ctrl, clk_ddr, clk_mesh, clk_mesh};
    wire [NS-1:0] srstn = {aresetn_ctrl, aresetn_ctrl, aresetn_ddr,
                           aresetn_mesh, aresetn_mesh};
...
`define SB1_AXI(I, W) \
    ...
    sb_nmu #(.MW(32), .MIDW(MAXID), .AW(AW), .FW(FW), .TAGW(TAGW),
             ...
             .STORE_FWD(STORE_FWD), .NSEG(NSEG0), .REQ_DEPTH(16), .RSP_DEPTH(256),
        ...
        `SB1_AXI(0, 32), .bus_clk(bus_clk), .bus_rst(bus_rst),
    ...
    sb_nmu #(.MW(32), .MIDW(MAXID), .AW(AW), .FW(FW), .TAGW(TAGW),
             .DSTW(DSTW), .LUT_PER_BRAM(LUT_PER_BRAM),
             .STORE_FWD(STORE_FWD), .NSEG(NSEG2), .REQ_DEPTH(16), .RSP_DEPTH(16),
        ...
        .s_aclk(clk_xdma), .s_aresetn(aresetn_xdma),
        `SB1_AXI(2, 32), .bus_clk(bus_clk), .bus_rst(bus_rst),
        `SB1_FLIT(2), .stat_decerr(dc2)
    );
