...
module sb_link_cdc #(
    ...
    always @(posedge i_clk) irst_q <= i_rst;
    always @(posedge o_clk) orst_q <= o_rst;
    ...
    always @(posedge i_clk)
        if (irst_q)   sent_cnt <= {CW{1'b0}};
        else if (send) sent_cnt <= sent_cnt + 1'b1;
...
`ifndef SYNTHESIS
    always @(posedge i_clk)
        if (!irst_q && pipe_v[PIPE-1] && rxf_full)
