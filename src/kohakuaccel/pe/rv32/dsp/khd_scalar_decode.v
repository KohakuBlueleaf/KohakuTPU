// khd_scalar_decode -- the three things the SCALAR pipeline needs to know about
...
// generated decode header: rv_id instantiates this under its DSP_EN generate
// and a build without the extension does not include khd_isa.vh at all.

`default_nettype none

module khd_scalar_decode #(
...
`include "khd_isa.vh"
    ...
    wire is_int_maj = (instr[6:0] == KHD_OPCODE);
    wire is_flt_maj = (HAS_F16 != 0) && (instr[6:0] == KHF_OPCODE);
    ...
    assign use_rs1 = is_int_maj && ((f3 == KHD_F3_VLD) || (f3 == KHD_F3_VST)
                                || ((f3 == KHD_F3_VMOV) && (f7 == KHD_MOV_SPLAT)));

    assign is_vld = is_int_maj && (f3 == KHD_F3_VLD);

    assign wr_rd = (is_int_maj && (f3 == KHD_F3_VMOV)
                    && ((f7 == KHD_MOV_EXTR) || (f7 == KHD_MOV_REDSUM)
                        || (f7 == KHD_MOV_REDMAX)))
