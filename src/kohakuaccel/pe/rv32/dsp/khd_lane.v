// khd_lane -- one 32-bit lane of the DSP datapath: the packed ALU, the packed
...
module khd_lane #(
    ...
    // ---- the multipliers: products at +1, the dot sum at +2 ----
    input  wire        mul_en,
    output wire [33:0] dot_sum,     // registered, valid two cycles after a, b
    ...
    wire [31:0] rnd_y;
    khd_padd32 u_rnd (
        .a(sh_y), .b(sh_round_bit), .sub(1'b0), .et(et), .mask(el_mask),
        .sat(1'b0),
        .y(rnd_y), .lt(), .lt_s(), .top()
    );
    ...
    khd_mul #(.AW(17), .BW(17), .PW(34), .USE_DSP(USE_DSP)) u_m0 (
        .clk(clk), .en(mul_en), .a(m0a), .b(m0b), .p(p0));
    khd_mul #(.AW(17), .BW(17), .PW(34), .USE_DSP(USE_DSP)) u_m1 (
        .clk(clk), .en(mul_en), .a(m1a), .b(m1b), .p(p1));

    reg [1:0] et_r;
    always @(posedge clk) if (mul_en) et_r <= et;

    wire signed [33:0] hi;
    generate
    if (MULS >= 4) begin : g_int8
        khd_mul #(.AW(9), .BW(9), .PW(18), .USE_DSP(USE_DSP)) u_m2 (
            .clk(clk), .en(mul_en), .a(m2a), .b(m2b), .p(p2));
        khd_mul #(.AW(9), .BW(9), .PW(18), .USE_DSP(USE_DSP)) u_m3 (
            .clk(clk), .en(mul_en), .a(m3a), .b(m3b), .p(p3));
        assign hi = (et_r == ET_S8)
                  ? ({{16{p2[17]}}, p2} + {{16{p3[17]}}, p3}) : 34'sd0;
    end else begin : g_no_int8
        // MULS = 2 has no int8 dot at all; khd_unit refuses the encoding, so
    ...
    always @(posedge clk) sum_r <= p0 + p1 + hi;
