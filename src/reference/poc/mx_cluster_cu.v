...
module mx_cluster_cu #(
    ...
    // Only u_node's L1 and cascade see clk2x; the NoC port and every sequencer
    // here stay on clk.
    wire clk = clk1x;
...
`endif
    wire         gemm_busy;
    // Combinational 2x -> 1x, vld_sr to the FSM's CE: 1.781 ns capped clk2x at
    // 561 MHz. Safe to register, exactly as peer_open_r below is.
    reg          gemm_busy_r, sweep_busy_r;
    always @(posedge clk) begin
        if (!resetn) begin gemm_busy_r <= 1'b0; sweep_busy_r <= 1'b0; end
        else         begin gemm_busy_r <= gemm_busy; sweep_busy_r <= sweep_busy; end
    end
    ...
    mx_cluster_node #(.TILES(TILES), .GA(GA), .GB(GB),
                      .ACC_MW(ACC_MW), .MODEL(MODEL), .PUMP(PUMP), .L1_PRIM(L1_PRIM),
                      .TILE_PRIM(TILE_PRIM)) u_node (
        .clk1x(clk), .clk2x(clk2x), .rst(!resetn),
    ...
    always @(posedge clk) begin
        if (!resetn) begin
            ...
            case (st)
                ...
                S_GEMM: if (
                    !(gemm_wide ? sweep_busy_r : gemm_busy_r)
                    && (!emit_r || !drain_busy)
                    && !peer_open
                ) begin
                    gemm_start <= 1'b1;
                ...
                S_DRAIN: if ((fuse_r || !gemm_busy_r) && !peer_open) begin
                    drain_n     <= n_r;
                    drain_start <= 1'b1;
                    st <= S_DWAIT;
                end
    ...
    always @(posedge clk) begin
        if (!resetn) begin
            ...
            for (wbi = 0; wbi < 2*WBURST; wbi = wbi + 1) begin
                w_buf[wbi] <= 256'd0;
            end
        end else begin
            if (w_take) begin
...
`ifndef SYNTHESIS
    ...
    always @(posedge clk) begin
        if (resetn && send_valid && (^send_flit[255 -: 34] === 1'bx)) begin
            $display("%0t ERROR mx_cluster_cu: read request address is x", $time);
        end
        if (resetn && w_valid && (w_st == 2'd2) && (^w_flit[255 -: 34] === 1'bx)) begin
        ...
        if (resetn && pl_valid && (pl_word != 2'd0) && (pl_ent !== asm_ent)) begin
        ...
        if (resetn && cd_pop && !cd_run && (cd_dbuf == BUF_PEER) && cd_doff[0]) begin
            $display("%0t ERROR mx_cluster_cu: peer stream starts at odd granule %0d -- a sub-tile is two flits",
                     $time, cd_doff);
        end
        if (resetn && cd_pop && !cd_run && !cd_fits) begin
        ...
        if (resetn && recv_valid && recv_ready && !rx_resp && !rx_data) begin
