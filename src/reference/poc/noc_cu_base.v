...
module noc_cu_base #(
    ...
    input  wire                   clk,
    // ASYNC=1 puts the NoC port on its own clock. The two inbound queues become
    // dual-clock; nothing else in this module changes domain.
    input  wire                   clk_noc,
    input  wire                   resetn,
    ...
    wire [$clog2(INST_DEPTH):0] inst_wrcnt;
    wire inst_ovf, recv_ovf;

    generate if (ASYNC != 0) begin : g_inst_async
        async_fifo #(.DATA_WIDTH(FLIT_WIDTH), .FIFO_DEPTH(INST_DEPTH),
                     .MEMORY_TYPE(MEM_TYPE)) u_inst (
            .wr_clk(clk_noc), .rd_clk(clk), .rst(!resetn),
            .wr_en(noc_in_valid && !noc_in_busy && in_inst),
            .wr_data(noc_in_data), .wr_busy(inst_full),
            .wr_overflow(inst_ovf), .wr_count(inst_wrcnt),
            .rd_en(inst_pop), .rd_data(inst_head), .rd_busy(inst_empty)
        );
        assign inst_almost = inst_full;
    end else begin : g_inst_sync
        ...
        assign inst_wrcnt = {($clog2(INST_DEPTH)+1){1'b0}};
        assign inst_ovf   = 1'b0;
    end endgenerate
    ...
    generate if (ASYNC != 0) begin : g_recv_async
        async_fifo #(.DATA_WIDTH(FLIT_WIDTH), .FIFO_DEPTH(RECV_DEPTH),
                     .MEMORY_TYPE(RECV_MEM)) u_recv (
            .wr_clk(clk_noc), .rd_clk(clk), .rst(!resetn),
            .wr_en(noc_in_valid && !noc_in_busy && !in_inst && !in_ctrl),
            .wr_data(noc_in_data), .wr_busy(recv_full),
            .wr_overflow(recv_ovf), .wr_count(),
            .rd_en(recv_valid && recv_ready), .rd_data(recv_flit),
            .rd_busy(recv_empty)
        );
        assign recv_almost = recv_full;
    end else begin : g_recv_sync
    ...
    // `inst_used` counts a clk_noc write and a clk pop in ONE always block: a
    // two-clock counter under ASYNC, so credits drift silently. Use the FIFO's.
    assign inst_space = (ASYNC != 0)
                      ? (INST_DEPTH[15:0] - {{(16-$clog2(INST_DEPTH)-1){1'b0}}, inst_wrcnt})
                      : (INST_DEPTH[15:0] - inst_used);
