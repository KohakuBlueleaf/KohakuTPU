...
// POC ONLY. Shipping file is src/kohakutpu/matmul/mx_cluster_mgr.v.
// One L1 entry per clk2x cycle is two K blocks per clk cycle: the double pump.
module mx_cluster_mgr #(
    ...
    // 1 = read TWO K blocks per clk cycle, one per clk2x phase. The cascade then
    // emits two partials per clk cycle, which the ACU cannot yet absorb (s10.2b).
    parameter integer PUMP   = 0,
    parameter integer PH_INV = 0
)(
    input  wire         clk,
    input  wire         clk2x,
    input  wire         rst,
    ...
    // its own delayed copy for exactly the FIRST 2x cycle of each 1x cycle.
    ...
    wire ph1 = PUMP && (!(tog_q ^ tog_qq) ^ PH_INV[0]);

    wire [AAW-1:0] a_rd2 = a_rd + {{(AAW-1){1'b0}}, ph1};
    wire [BAW-1:0] b_rd2 = b_rd + {{(BAW-1){1'b0}}, ph1};
    ...
    reg [31:0]    s1b_sa, s1b_sb, s1b_sa2, s1b_sb2;
...
`ifndef SYNTHESIS
    // PUMP_TRACE dumps the 2x read stream: address, phase, and the first
    // element of each operand, so a mispaired K block is visible directly.
    always @(posedge clk2x) if (PUMP != 0 && !rst && $time < 200000 && core_valid)
        $display("%0t 2x ph=%0b a_rd=%0d a_rd2=%0d a0=%0d b0=%0d v=%0b",
                 $time, ph1, a_rd, a_rd2, $signed(a_out[6:0]),
                 $signed(b_out[6:0]), core_valid);

    // An odd NK pumped reads one block past the operand: the pair's second half
    // is entry kb+1, which for the last odd block is the NEXT sweep's data.
    initial if (PUMP) $display("NOTE mx_cluster_mgr: PUMP requires an EVEN gemm_nk");
    always @(posedge clk) if (!rst && gemm_start && PUMP && gemm_nk[0])
        $display("%0t ERROR mx_cluster_mgr: PUMP with odd nk=%0d reads past the operand",
                 $time, gemm_nk);
...
`endif
    ...
    // Two K blocks per issue when pumped, so the counter steps by two and the
    // last-block test has to match it or the sweep re-reads what it just summed.
    localparam [7:0] KSTEP = PUMP ? 8'd2 : 8'd1;
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            // Scales ride the issue pipeline: sa_p0/sa_p1 are one shared pair at
            // the clk2x rate, overwritten by issue N+1 before N is pushed.
            s1b_sa  <= sa_p0; s1b_sb  <= sb_p0;
            s1b_sa2 <= sa_p1; s1b_sb2 <= sb_p1;

            s2_valid <= s1b_valid;
            s2_first <= s1b_first;
            s2_emit  <= s1b_emit;
            s2_addr  <= s1b_addr;
            // sa pairs with part_in, which carries the SECOND phase; sa2 with
            // part_in2, the first. Swapped, each partial gets the other's E5M3.
            // sa_p1 direct, sa_p0 pipelined: phase 1's scale lands half a clk
            // after phase 0's, so one shared sample point catches one of them stale.
            s2_sa    <= (PUMP != 0) ? sa_p1 : a_ent[927:896];
            s2_sb    <= (PUMP != 0) ? sb_p1 : b_ent[927:896];
            s2_sa2   <= s1b_sa;
            s2_sb2   <= s1b_sb;
        end
    end
...
`ifndef SYNTHESIS
        if (PUMP != 0 && !rst && $time < 260000 && core_valid)
            $display("%0t SCAP ph1=%0b ph1_d=%0b a_rd2=%0d aent_s=%h p0=%h p1=%h",
                     $time, ph1, ph1_d, a_rd2, a_ent[927:896], sa_p0, sa_p1);
`endif
    end
