# multimesh v8t: four stations, four nodes on ONE sysnode clock, ONE
# partition-aware Kohaku Xache, four MIGs, NO mesh.
#   vivado -mode batch -source scripts/tcl/multimesh_v8t_bd.tcl \
#     ?-tclargs rebuild|synth ?jobs N??
# Every knob is in scripts/tcl/v8t/00_config.tcl and nowhere else. Its own
# project, always; the v7/v8 flows and their projects are untouched. No
# implementation step: the goal ends at an analysed synthesis.
...
puts "@@@ v8t mref: dropped [llength $v8_dropped] stale module-reference cache(s)"
...
source $here/v8t/10_sources.tcl

create_bd_design $design_name
current_bd_design $design_name

source $here/v8t/20_clocks.tcl
source $here/v8t/30_nodes.tcl
source $here/v8t/35_xache.tcl
source $here/v8t/40_bus.tcl
source $here/v8t/50_addr.tcl

regenerate_bd_layout
validate_bd_design
source $here/v8t/55_addr_fill.tcl
validate_bd_design
save_bd_design

source $here/v8t/60_constraints.tcl
...
puts "@@@ v8t top: [get_property top [current_fileset]] in $proj_dir"

}

if {!$do_synth} {
    puts "@@@ v8t: block design built. Verify with v8t/75_verify_bd.tcl; -tclargs synth to synthesise."
    return
}

# A run whose launcher died keeps STATUS "Running" in the project forever and
# refuses a relaunch (Common 17-69); reset anything unfinished before launching.
...
if {[llength $ooc]} {
    puts "@@@ v8t ooc: [llength $ooc] runs, -jobs $OOC_JOBS"
    ...
    puts "@@@ v8t ooc: every IP run already complete"
}

if {[llength [v8_pending synth_1]]} {
    puts "@@@ v8t synth_1: launched at [clock format [clock seconds] -format %H:%M:%S]"
    launch_runs synth_1 -jobs $njobs
    wait_on_run synth_1
} else {
    puts "@@@ v8t synth_1: already complete"
}
if {[get_property PROGRESS [get_runs synth_1]] != "100%"} {
    error "synthesis failed: $proj_dir/${design_name}.runs/synth_1/runme.log"
}
puts "@@@ v8t synth_1: done at [clock format [clock seconds] -format %H:%M:%S]"
source $here/v8t/70_analyze.tcl
puts "@@@ v8t: synthesis and analysis done. No implementation in this flow."
