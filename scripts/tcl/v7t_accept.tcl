...
proc chk {name got want} {
    global out fail nchk
    incr nchk
    if {$got ne $want} {
        incr fail
        lappend out "  CHECK $name: got $got want $want  FAIL"
    } else {
        lappend out "  CHECK $name: ok"
    }
}
proc wr1 {addr beat} { jaxi::write $addr [list $beat] }

# ---- A: wizard reads, every word, all four --------------------------------
lappend out "=== A: wizard register reads (all lanes) ==="
...
lappend out "=== D-pre: endpoints on built clocks ==="
for {set m 0} {$m < 4} {incr m} { ep_pass $m pre }

# ---- half-word writes via rd-merge-wr at port2 (real Lite) ----------------
lappend out "=== D-half: 32-bit halves at port2 via jaxi::wr32 ==="
set lite0 0x000080
...
if {[catch {jaxi::write [expr {$lite0 + 0x0C}] {0000000012345678}} e]} {
    lappend out "  INFO unaligned write: refused ($e)"
} else {
    lappend out "  INFO unaligned write: accepted, w0x88=[rdw [expr {$lite0 + 8}]]"
}

# ---- B: the historically fatal shapes, real values ------------------------
# Duty halves carry 50000 (0xC350): duty=0 is the value the wizard SLVERRs.
lappend out "=== B: the v6.7-fatal word writes ==="
set W0 0x900000
...
lappend out "=== C: mid-profile retune (6/3/6/6) on all wizards ==="
for {set m 0} {$m < 4} {incr m} {
    set W [expr {0x900000 + $m * 0x10000}]
    ...
    lappend out "  wiz$m LOAD issued, status=[rdw $W]"
}

# ---- D-post: endpoints alive at the NEW clocks ----------------------------
lappend out "=== D-post: endpoints after retune ==="
after 100
for {set m 0} {$m < 4} {incr m} { ep_pass $m post }

lappend out "==================================================="
lappend out [format "checks=%d fail=%d" $nchk $fail]
lappend out [expr {$fail == 0 ? "V7T-ACCEPT-PASS" : "V7T-ACCEPT-FAIL"}]
join $out "\n"
