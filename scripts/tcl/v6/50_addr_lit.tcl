# Segment table and literal encoding, sourced by BOTH 50_addr.tcl and
# check_map.tcl. sb_nmu: MASK 1 = compare, emitted = (a & ~MASK) | XLT.
...
# Segment k = station*NQ + port, in index order.
...
# not hex, because AW=43 is not a multiple of 4. Segment k is at [k*W +: W],
# so k=0 is the LSB and the string runs most-significant segment first.
proc v6_cat {vals width} {
    set s ""
    foreach v [lreverse $vals] { append s [v6_bits $v $width] }
    return "0b$s"
}
