...
foreach {mid mod} $MESHES {
    ...
    if {![llength $kxseg]} { error "v8t3 addr: the Xache's S0${mid}_AXI has no segment to assign" }
    ...
    if {![llength $kxsp]} { error "v8t3 addr: the Xache's M0${mid}_AXI has no address space" }
    assign_bd_address -target_address_space $kxsp \
        [get_bd_addr_segs ddr4_$mid/C0_DDR4_MEMORY_MAP/C0_DDR4_ADDRESS_BLOCK] \
        -offset 0x0 -force
    incr v8_nmap
}
puts "@@@ v8t3 addr fill: [llength [get_bd_addr_segs -quiet -excluded]] excluded,\
