# RTL for multimesh v8t3. Named, not globbed: a stray file under src/ must not
...
set V8_SOURCES {
    ...
    src/kohakutransmit/packet/kts_pkt.vh
...
if {[llength $v8_missing]} {
    error "v8t3 sources missing: [join $v8_missing {, }]"
}
# A .vh is a global include or the module that `include`s it never finds it:
# kx_kedge's kts_pkt.vh failed the Xache's out-of-context run this way.
foreach f $V8_SOURCES {
    if {[string match *.vh $f]} {
        set_property file_type {Verilog Header} [get_files $root/$f]
        set_property is_global_include true     [get_files $root/$f]
    }
}
update_compile_order -fileset sources_1
puts "@@@ v8t3 sources: [llength $V8_SOURCES] files"
