...
# the reset audit on both trees, the station bus. V8_MODE ro (v8t3_analyze.tcl)
...
open_run synth_1 -name ${design_name}_synth
set out $root/build
file mkdir $out
set top ${design_name}_i
proc v8_slurp {path} {
    if {![file exists $path]} { error "v8t3 analyze: $path missing" }
...
if {[llength [get_cells -quiet $top/clk_wiz_bus*]]} { v8_bad "a per-die bus wizard exists: the bus clocks come off clk_wiz_ctrl" }
# Every die reaches its own sysnode and bus clock, and at PER_DIE_CLK 1 no two
# dies reach the same one.
set seen [dict create]
foreach {mid mod} $MESHES {
    foreach k [list [v8_sys_clk $mid] [v8_bus_clk $mid]] {
        if {$PER_DIE_CLK && [dict exists $seen $k]} {
            v8_bad "die $mid shares $k with die [dict get $seen $k]"
        }
        dict set seen $k $mid
    }
}
...
puts $lfh "# GENERATED by scripts/tcl/v8t3/70_analyze.tcl: Xache leaves hoisted out of their scope"
set unresolved 0 ; set pinned 0 ; set ring 0
foreach c $unclaimed_cells {
    # At KX_PCLK 1 a boundary ring is an async FIFO with registers in BOTH
    # dies, so 60_constraints pins it to neither and its leaves have no single
    # die by construction -- the placer puts it at the boundary.
    if {$KX_PCLK && [string match "*.u_tk/g_ring*" [get_property NAME $c]]} {
        incr ring ; continue
    }
    set pbs {}
...
puts "  hoisted leaves pinned $pinned (in $leafxdc), unresolved $unresolved,\
 boundary-ring leaves left free $ring"
...
puts "\n=== meshes === [v8_t]"
proc v8_sysclk_name {mid} {
    global top
    return [get_property NAME [get_clocks -of_objects [get_pins $top/[v8_sys_clk $mid]]]]
}
set sysclk_name [v8_sysclk_name 0]
foreach {mid mod} $MESHES {
    set sysclk_name [v8_sysclk_name $mid]
    ...
    set bits {}
    foreach p {MESH_ID ILINK PORTS L2_MAG_BANKS L2_MAG_ENTRIES DRAM_CDC DRAM_AR_MAX} {
...
puts "\n=== the Xache's clocks ==="
foreach pin {p_clk0 p_clk1 p_clk2 p_clk3 h_clk0 h_clk1 h_clk2 h_clk3} {
    ...
    if {[regexp {^p_clk(\d)$} $pin -> pi] && $nm ne [v8_sysclk_name $pi]} {
        v8_bad "xache/$pin is $nm, not die $pi's sysnode clock [v8_sysclk_name $pi]"
    }
}
foreach {mid mod} $MESHES {
...
# The clock pins of a HIERARCHICAL cell carry no clock after synthesis, so this
# reads the leaf registers inside it -- asking the hierarchy returns an empty
# list and the count is 0 whatever the design does.
proc v8_clocks_in {cell} {
    set r {}
    set seq [get_cells -quiet -hier -filter "NAME =~ [v8_glob $cell]/* && IS_SEQUENTIAL"]
    foreach cp [get_pins -quiet -leaf -of_objects $seq -filter {IS_CLOCK}] {
        set k [get_clocks -quiet -of_objects $cp]
        if {[llength $k]} { lappend r [get_property NAME [lindex $k 0]] }
    }
...
proc v8_count_offclock {cells want} {
    set bad 0
    foreach h $cells {
        foreach c [v8_clocks_in $h] { if {$c ne $want} { incr bad } }
    }
    return $bad
}
...
# u_tx runs on the die the hop LEAVES, u_rx on the die it lands on; at
# PER_DIE_CLK 0 those are the same net.
set pipebad 0
foreach h $pipecells {
    if {![regexp {pipe_(\d)_to_(\d)/inst/u_(tx|rx)$} $h -> lo hi half]} { continue }
    set want [v8_sysclk_name [expr {$half eq "tx" ? $lo : $hi}]]
    foreach c [v8_clocks_in $h] {
        if {$c ne $want} { incr pipebad ; puts "  OFFCLOCK $h on $c, want $want" }
    }
}
puts "  pipe halves [llength $pipecells], on another clock $pipebad"
if {$pipebad} { v8_bad "$pipebad pipe half(s) not on their die's sysnode clock" }
...
# At IL_ASYNC 1 the peer is not the other half -- kts_cdc's forward FIFO takes
# u_tx's registers and its backward FIFO takes u_rx's, and the crossing wire is
# a register driving that FIFO's write port.
foreach h $pipecells {
    if {$IL_ASYNC} {
        set fifo [expr {[string match "*/u_tx" $h] ? "u_f" : "u_b"}]
        set peer "[file dirname $h]/g_async.u_x/$fifo"
    } else {
    ...
    # A Laguna pair is one register driving one register. At IL_ASYNC 1 the far
    # end is a FIFO write port, which is not a Laguna site, so only reachability
    # is asserted.
    if {$multi && !$IL_ASYNC} { v8_bad "$h: $multi crossing register(s) drive more than one load -- not a Laguna pair" }
    if {!$xbits} { v8_bad "$h: no register reaches [file tail $peer]" }
}
...
# ctrl + 4 MIG, plus one bus and one sys reset -- per die at PER_DIE_CLK 1.
set want_psr [expr {$PER_DIE_CLK ? 13 : 7}]
if {[llength $psrs] != $want_psr} { v8_bad "[llength $psrs] proc_sys_reset blocks under $top, want $want_psr" }
foreach psr $psrs {
...
foreach t [expr {$PER_DIE_CLK ? {} : {rst_tree rst_tree_bus}}] {
    puts "\n=== $t === [v8_t]"
    foreach {mid mod} $MESHES {
...
set bclk [get_clocks -quiet -of_objects [get_pins -quiet $top/[v8_bus_clk 1]]]
if {![llength $jclk] || ![llength $bclk] || [get_property NAME $jclk] ne [get_property NAME $bclk]} {
    v8_bad "jtag_ctrl/aclk is not station 1's bus clock"
}
# A module reference's inner ref names carry the reference's prefix
# (`multimesh_v8t3_station_bus_0_sb_link__xdcDup__1`), so match `*sb_link*`.
...
if {$LINK_CDC && $nlcdc != 6} { v8_bad "$nlcdc sb_link_cdc instance(s), want 6: LINK_CDC did not take" }
if {!$LINK_CDC && $nlcdc} { v8_bad "$nlcdc sb_link_cdc instance(s): LINK_CDC did not take" }
if {[catch {open_bd_design [get_files ${design_name}.bd]} bdmsg]} {
    ...
    if {[get_property -quiet CONFIG.LINK_CDC $sbbd] ne "$LINK_CDC"} { v8_bad "station_bus LINK_CDC is not $LINK_CDC" }
    current_design ${design_name}_synth
}
...
if {[llength $wp]} {
    puts "@@@ v8t3 synth WNS [format %.3f [get_property SLACK $wp]] levels [get_property LOGIC_LEVELS $wp]"
...
puts "\n@@@ reports in $out  [v8_t]"
if {$v8_fail} { error "$design_name analysis:$v8_fail check(s) FAILED" }
puts "@@@ v8t3 analysis: all checks passed"
