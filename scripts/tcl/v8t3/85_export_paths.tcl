...
open_run impl_1 -name ${design_name}_export
set out $root/build
...
puts "@@@ per-clock written"

# ---- 7. CONGESTION, and who owns the cells inside each window -------------
# The level is the metric, not WNS: L4 is the target and L5 the floor with no
# mesh. report_design_analysis names a window by its site range, so the cells
# in that range are the answer to "which part congests".
report_design_analysis -congestion -extend -file $out/${design_name}_congestion_extend.rpt
report_design_analysis -complexity -hierarchical_depth 4 \
    -file $out/${design_name}_complexity_d4.rpt

# Every placed primitive: LOC, its clock region, and the module that owns it.
# Binned offline (scripts/py/cellmap.py) into per-region and per-column grids.
set f [open $out/${design_name}_cell_loc.tsv w]
puts $f [join {LOC REGION OWNER NAME} \t]
set nc 0
foreach c [get_cells -quiet -hier -filter {IS_PRIMITIVE && LOC != ""}] {
    set nm  [get_property NAME $c]
    set loc [get_property LOC $c]
    set st  [get_sites -quiet $loc]
    set cr  [expr {[llength $st] ? [get_property -quiet CLOCK_REGION $st] : "-"}]
    puts $f [join [list $loc $cr [v8_owner $nm] $nm] \t]
    incr nc
}
close $f
puts "@@@ cell_loc: $nc placed primitives with clock region and owner"

# Nets crossing an SLR, by owner: the SLL budget actually spent.
array unset X
foreach n [get_nets -quiet -hier -filter {CROSSING_SLRS != ""}] {
    incr X([v8_owner [get_property NAME $n]])
}
set f [open $out/${design_name}_sll_owner.tsv w]
puts $f [join {OWNER NETS_CROSSING_SLR} \t]
foreach k [lsort [array names X]] { puts $f "$k\t$X($k)" }
close $f
puts "@@@ ALL exports done"
