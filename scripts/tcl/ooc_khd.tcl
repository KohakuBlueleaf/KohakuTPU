# OOC synthesis of the KohakuDSP vector unit. SYNTH ONLY. Results are the @@@ lines.
#   vivado -mode batch -source scripts/tcl/ooc_khd.tcl -tclargs \
...
# of making a controller PE a DSP PE is what the configuration matrix is about,
...
set tag "khd-s$simd-v$vreg-a$nacc-e$vspe-m$muls-sh$hsh-pm$hpm-wb$wbs-f$f16-p$npt-$udsp-$vprm-t$per"
puts "@@@ top khd_unit simd $simd vregs $vreg nacc $nacc vspad $vspe muls $muls shift $hsh perm $hpm wb $wbs f16 $f16 npart $npt use_dsp $udsp vreg_prim $vprm period $per"
...
synth_design -top khd_unit -part $part -mode out_of_context \
             ...
             -generic HAS_F16=$f16 -generic NPART=$npt \
...
if {$f16 != 0} {
    lappend insts "g_f16.u_facc" "g_f16.u_ffold"
    foreach s {0 1} {
        lappend insts "g_f16.g_flane\[$s\].u_fl" \
                      "g_f16.g_flane\[$s\].u_fl/u_alu" \
                      "g_f16.g_flane\[$s\].u_fl/u_ca" \
                      "g_f16.g_flane\[$s\].u_sd" \
                      "g_f16.g_fout\[$s\].u_o"
    }
}
foreach inst $insts {
