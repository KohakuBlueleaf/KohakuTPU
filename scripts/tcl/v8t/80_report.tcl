# v8t post-route report: per-SLR resources, SLL per boundary, WNS per clock,
# the worst path. Run from v8t_impl.tcl or alone:
#   vivado -mode batch -source scripts/tcl/v8t/80_report.tcl
if {![info exists design_name]} {
    set here [file dirname [file normalize [info script]]]
    source $here/00_config.tcl
    open_project $proj_dir/${design_name}.xpr
}
open_run impl_1 -name v8t_impl
set out $root/build
file mkdir $out
...
# Every net that crosses a die: per boundary, and per owner so the station
# line, the reset tree and the Xache's lanes are told apart.
puts "\n=== SLL per boundary ==="
