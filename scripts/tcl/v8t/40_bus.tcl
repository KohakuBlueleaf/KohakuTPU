...
# fixed by sb_line4 port_dom(): 0,1 the station's port clock (the ONE sysnode
# clock in v8t); 2 clk_ddr; 3 clk_ctrl. Each station's fabric runs on ITS die's
# bus clock; the links between stations cross through sb_link_cdc.
...
foreach {mid mod} $MESHES {
    ...
    # Port 2 of each station runs on THAT SLR's MIG ui_clk, so the station
    # meets the controller in its own domain and no crossing IP sits between.
...
# and 64-bit data, stated, not defaulted (the IP defaults to 32).
...
v8_jtag jtag_ctrl clk_wiz_ctrl/clk_out1 rst_ctrl/peripheral_aresetn
...
foreach {mid mod} $MESHES {
    ...
    connect_bd_intf_net [get_bd_intf_pins station_bus/$p0] \
                        [get_bd_intf_pins node_$mid/S_AXI_MEM]
    ...
    connect_bd_intf_net [get_bd_intf_pins dwc_ctrl$mid/M_AXI] \
                        [get_bd_intf_pins node_$mid/S_AXI_CTRL]
    ...
    # The die's clock controller, reached on the FIXED ctrl clock so a retune
    # never stands on the clock it is changing.
...
puts "@@@ v8t bus: 3 managers, [expr {4 * $NQ}] endpoints, every die real"
