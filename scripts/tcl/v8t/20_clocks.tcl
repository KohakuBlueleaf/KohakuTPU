...
# Per die: the controllable wizard the station's port 3 reaches (v7t's
# convention, one per die). Its first output is the unloaded mesh-rate output
# that keeps the board file's indices; on SYS_WIZ alone the fourth output is
# THE sysnode clock. No die has a noc, mat or vec domain in v8t.
foreach {mid mod} $MESHES {
    set outs [list [list 1 $MESH_MHZ $DIV_MESH]]
    if {$mid == $SYS_WIZ} {
        lappend outs [list 2 $MESH_MHZ $DIV_MESH] [list 3 $MESH_MHZ $DIV_MESH] \
                     [list 4 $SYS_MHZ $DIV_SYS]
    }
...
puts "@@@ v8t clocks: ctrl, sys (mesh$SYS_WIZ/clk_out4), 4 die wizards, 4 bus wizards; resets ctrl, sys, 4 bus (MIG resets in 30_nodes)"
