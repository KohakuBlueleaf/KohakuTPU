...
# Control: fixed, no DRP. out1 100 MHz control plane, out2 200 MHz system.
v8_wiz clk_wiz_ctrl [list [list 1 $CTRL_MHZ $DIV_CTRL] [list 2 $BUS_MHZ $DIV_BUS]] 0
v8_psr rst_ctrl $CTRL_CLK clk_wiz_ctrl/locked
v8_psr rst_bus  $BUS_CLK  clk_wiz_ctrl/locked
v8_tree rst_tree_bus $BUS_CLK rst_bus/peripheral_aresetn

# Per die: NoC, matmul 2x, vector -- and on SYS_WIZ only, the sysnode clock
# as a fourth output. The NoC, mat and vec domains take their resets INSIDE
# the top (kh_rst_sync from axi_aresetn); a proc_sys_reset here would have no
# load (v8: measured 0).
...
v8_tree rst_tree $SYS_CLK rst_sys/peripheral_aresetn
...
puts "@@@ v8t2 clocks: ctrl (100 + 200), sys (mesh$SYS_WIZ/clk_out4), 4 mesh wizards; resets ctrl, bus (+tree), sys (+tree); MIG resets in 30_meshes"
