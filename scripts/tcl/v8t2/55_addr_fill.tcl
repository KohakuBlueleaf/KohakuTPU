...
#   mesh m's DRAM master  -> the Xache's S0m_AXI aperture, at 0
#   the Xache's M0h_AXI   -> ddr4_h, the MIG in SLR h, at 0 (4 GB)
...
foreach {mid mod} $MESHES {
    ...
    if {![llength $kxseg]} { error "v8t2 addr: the Xache's S0${mid}_AXI has no segment to assign" }
    assign_bd_address -target_address_space [get_bd_addr_spaces mesh_$mid/M_AXI_DRAM] \
        $kxseg -offset 0x0 -force
    set kxsp [get_bd_addr_spaces -quiet xache/M0${mid}_AXI]
    if {![llength $kxsp]} { error "v8t2 addr: the Xache's M0${mid}_AXI has no address space" }
    assign_bd_address -target_address_space $kxsp \
        [get_bd_addr_segs ddr4_$mid/C0_DDR4_MEMORY_MAP/C0_DDR4_ADDRESS_BLOCK] \
        -offset 0x0 -force
    incr v8_nmap
}
puts "@@@ v8t2 addr fill: [llength [get_bd_addr_segs -quiet -excluded]] excluded,\
      $v8_nmap mesh->Xache->MIG paths assigned"
