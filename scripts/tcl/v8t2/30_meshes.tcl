...
# ---- meshes ----------------------------------------------------------------
if {$DRAM_CDC} { error "v8t2 is the one-clock memory path; DRAM_CDC must be 0" }
foreach {mid mod} $MESHES {
    ...
    set_property -dict [list CONFIG.MESH_ID $mid CONFIG.GA {512} CONFIG.GB {512} \
                             ...
                             CONFIG.DRAM_CDC $DRAM_CDC] [get_bd_cells mesh_$mid]
    ...
    # The host-stub window is the Verilator harness's; on the card the host
    # reaches the node through S_AXI_CTRL and S_AXI_MEM.
...
proc v8_pipe {name sp dp tx rx} {
    global SYS_CLK IL_W IL_VCW IL_CN_W
    ...
    set_property -dict [list CONFIG.W $IL_W CONFIG.VCW $IL_VCW CONFIG.CN_W $IL_CN_W] \
        [get_bd_cells $name]
    connect_bd_net [get_bd_pins $SYS_CLK] [get_bd_pins $name/clk]
...
puts "@@@ v8t2 meshes: [llength [get_bd_cells -quiet mesh_*]], 4 MIGs by SLR, [llength [get_bd_cells -quiet pipe_*]] interlink pipes on the sysnode clock"
