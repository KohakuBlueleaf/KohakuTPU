...
def ev(expr, dw):
    ...
    return eval(expr.replace("DW", str(dw)).replace("AW", str(AW))
                    .replace("/", "//"))
...
def emit_root(name, mgr_w, loc_w, nk):
    ...
    m_glue = "\n\n".join(axi_glue(i, p, w, "sp", True)
                         for i, (p, w) in enumerate(zip(m_names, loc_w)))
...
def _unused_emit_line(name, stn, nstn, mgr_w, loc_w):
    """A LINE station wrapper: left/right AXIS links, named AXI on both sides.

    A station with no managers still instantiates one injection port, tied off
    here, because sb_stn_line's hub needs NSRC >= 1.
    """
    nm, nq = max(1, len(mgr_w)), len(loc_w)
    rqw, rsw, stnw, portw, srcw = line_widths(nstn, nq, nm)
    prq, prs = pad8(rqw), pad8(rsw)
    s_names = [f"S{i:02d}_AXI" for i in range(len(mgr_w))]
    m_names = [f"M{i:02d}_AXI" for i in range(nq)]
    ...
    m_glue = "\n\n".join(axi_glue(i, p, w, "sp", True)
                         for i, (p, w) in enumerate(zip(m_names, loc_w)))
    loc_bits = sum((1 << i) for i, w in enumerate(loc_w) if w == FW)
    assoc_s = ":".join(s_names) if s_names else ""
    sblock = (s_decls + "\n\n") if s_decls else ""
    sclk = (f"""    (* X_INTERFACE_INFO = "xilinx.com:signal:clock:1.0 clk_mgr CLK" *)
    (* X_INTERFACE_PARAMETER = "ASSOCIATED_BUSIF {assoc_s}, ASSOCIATED_RESET aresetn_mgr" *)
    input  wire clk_mgr,
    (* X_INTERFACE_INFO = "xilinx.com:signal:reset:1.0 aresetn_mgr RST" *)
    (* X_INTERFACE_PARAMETER = "POLARITY ACTIVE_LOW" *)
    input  wire aresetn_mgr,

""" if s_names else "")

    return f"""// {name} -- GENERATED by scripts/py/gen_station_wrap.py. Do not edit by hand.
//
// Station {stn} of {nstn} on a line: left and right AXIS links, {len(mgr_w)} AXI
// slaves, {nq} AXI masters. The logic is in sb_stn_line.v.
...
module {name} #(
    ...
    (* X_INTERFACE_INFO = "xilinx.com:signal:clock:1.0 bus_clk CLK" *)
    (* X_INTERFACE_PARAMETER = "ASSOCIATED_BUSIF LF_REQ:LT_REQ:LF_RSP:LT_RSP:RF_REQ:RT_REQ:RF_RSP:RT_RSP, ASSOCIATED_RESET bus_rst" *)
    input  wire bus_clk,
    (* X_INTERFACE_INFO = "xilinx.com:signal:reset:1.0 bus_rst RST" *)
    (* X_INTERFACE_PARAMETER = "POLARITY ACTIVE_HIGH" *)
    input  wire bus_rst,

{sclk}    (* X_INTERFACE_INFO = "xilinx.com:signal:clock:1.0 clk_loc CLK" *)
    (* X_INTERFACE_PARAMETER = "ASSOCIATED_BUSIF {':'.join(m_names)}, ASSOCIATED_RESET aresetn_loc" *)
    input  wire clk_loc,
    (* X_INTERFACE_INFO = "xilinx.com:signal:reset:1.0 aresetn_loc RST" *)
    (* X_INTERFACE_PARAMETER = "POLARITY ACTIVE_LOW" *)
...
{axis_decls("LF_REQ", prq, False)}
{axis_decls("LT_REQ", prq, True)}
{axis_decls("LF_RSP", prs, False)}
{axis_decls("LT_RSP", prs, True)}
{axis_decls("RF_REQ", prq, False)}
{axis_decls("RT_REQ", prq, True)}
{axis_decls("RF_RSP", prs, False)}
{axis_decls("RT_RSP", prs, True)}

{sblock}{m_decls[:-1]}
);
    localparam integer NM   = {nm};
    localparam integer NQ   = {nq};
    localparam integer AW   = {AW};
    localparam integer MAXW = {MAXW};
    localparam integer MAXID = {MAXID};
    localparam integer FW   = {FW};
    localparam integer STNW = {stnw};
    localparam integer PORTW = {portw};
    localparam integer SRCW = {srcw};
    localparam integer RQW  = {rqw};
    localparam integer RSW  = {rsw};

    wire [NQ*MAXID-1:0]    sp_awid, sp_arid, sp_bid, sp_rid;
    wire [NQ*AW-1:0]       sp_awaddr, sp_araddr;
    wire [NQ*8-1:0]        sp_awlen, sp_arlen;
    wire [NQ*3-1:0]        sp_awsize, sp_arsize;
    wire [NQ*2-1:0]        sp_awburst, sp_arburst, sp_bresp, sp_rresp;
    wire [NQ-1:0]          sp_awvalid, sp_awready, sp_wvalid, sp_wready;
    wire [NQ-1:0]          sp_wlast, sp_bvalid, sp_bready;
    wire [NQ-1:0]          sp_arvalid, sp_arready, sp_rvalid, sp_rready, sp_rlast;
    wire [NQ*MAXW-1:0]     sp_wdata, sp_rdata;
    wire [NQ*(MAXW/8)-1:0] sp_wstrb;

{m_glue}

    // Station wiring is left to the integrator: this wrapper exposes the flit
    // ports so a BD can place the CDC links between stations as its own blocks.
    assign LF_REQ_tready = 1'b1;
    assign LF_RSP_tready = 1'b1;
    assign RF_REQ_tready = 1'b1;
    assign RF_RSP_tready = 1'b1;
    assign LT_REQ_tdata = {{{prq}{{1'b0}}}};
    assign LT_REQ_tvalid = 1'b0;
    assign LT_RSP_tdata = {{{prs}{{1'b0}}}};
    assign LT_RSP_tvalid = 1'b0;
    assign RT_REQ_tdata = {{{prq}{{1'b0}}}};
    assign RT_REQ_tvalid = 1'b0;
    assign RT_RSP_tdata = {{{prs}{{1'b0}}}};
    assign RT_RSP_tvalid = 1'b0;
endmodule
...
def main():
    ...
    ap.add_argument("--kind", choices=["root", "leaf", "link", "line4"],
                    required=True)
